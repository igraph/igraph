...
igraph_long_t print_vector(const igraph_vector_t *v) {
...
igraph_long_t print_matrix(const igraph_matrix_t *m) {
    igraph_long_t i, j, nrow = igraph_matrix_nrow(m), ncol = igraph_matrix_ncol(m);
    for (i = 0; i < nrow; i++) {
        for (j = 0; j < ncol; j++) {
