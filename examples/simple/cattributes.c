...
igraph_long_t print_attributes(const igraph_t *g) {
    ...
    for (i = 0; i < igraph_ecount(g); i++) {
        igraph_long_t j;
        printf("Edge %li (%i-%i): ", i, (igraph_long_t)IGRAPH_FROM(g, i), (igraph_long_t)IGRAPH_TO(g, i));
...
igraph_long_t main() {
    ...
    for (i = 0; i < igraph_strvector_size(&gnames); i++) {
        printf("%s (%i) ", STR(gnames, i), (igraph_long_t)VECTOR(gtypes)[i]);
    }
    printf("\n");
    printf("Vertex attributes: ");
    for (i = 0; i < igraph_strvector_size(&vnames); i++) {
        printf("%s (%i) ", STR(vnames, i), (igraph_long_t)VECTOR(vtypes)[i]);
    }
    printf("\n");
    printf("Edge attributes: ");
    for (i = 0; i < igraph_strvector_size(&enames); i++) {
        printf("%s (%i) ", STR(enames, i), (igraph_long_t)VECTOR(etypes)[i]);
    }
    printf("\n");
