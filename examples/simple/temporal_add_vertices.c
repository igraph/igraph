...
int main() {
  ...
  IGRAPH_VECTOR_TIME_CONSTANT(v_active_new, 3, 1, 0);

  igraph_create_temporal(&graph, &edges, 0, IGRAPH_DIRECTED, &e_active, 0,
			 &v_active, 0);

  igraph_add_vertices_at(&graph, 3, &v_active_new, 0, 0);

  igraph_add_vertices_at(&graph, 2, 0, 0, 0);

  igraph_time_goto(&graph, 2);
  igraph_add_vertices_at(&graph, 5, 0, 0, 0);

  igraph_time_goto(&graph, 0);
  igraph_add_vertices(&graph, 2, 0);

  igraph_vector_time_init(&active, 0);
  igraph_vertices_range(&graph, igraph_vss_all(), &active, 0);
