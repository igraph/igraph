...
int doit(int which) {
  ...
  for (i = 0; i < N; i++) { if (VECTOR(pos)[i] != i) return which + 2; }
  
  igraph_vector_null(&vec);
  colfun(&A, &vec, &pos);
  for (i = 0; i < N; i++) { if (VECTOR(vec)[i] != i) return which + 3; }
  for (i = 0; i < N; i++) { if (VECTOR(pos)[i] != i) return which + 4; }
  ...
  igraph_vector_null(&vec);
  rowfun(&A, &vec, &pos);
  for (i = 0; i < N; i++) { if (VECTOR(vec)[i] != i) return which + 5; }
  for (i = 0; i < N; i++) { if (VECTOR(pos)[i] != i) return which + 6; }
  
  igraph_vector_null(&vec);
  colfun(&A, &vec, &pos);
  for (i = 0; i < N; i++) { if (VECTOR(vec)[i] != i) return which + 7; }
  for (i = 0; i < N; i++) { if (VECTOR(pos)[i] != i) return which + 8; }
