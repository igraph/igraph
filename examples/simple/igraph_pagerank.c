...
int main() {
  ...
  igraph_pagerank(&g, &res, igraph_vss_all(), 1, 1000, 0.001, 0.85);
  ...
  ret=igraph_pagerank(&g, &res, igraph_vss_all(), 1, -1, 0.001, 0.85);
  if (ret != IGRAPH_EINVAL) {
    return 1;
  }
  
  ret=igraph_pagerank(&g, &res, igraph_vss_all(), 1, 1000, -1, 0.85);
  if (ret != IGRAPH_EINVAL) {
    return 2;
  }
  
  ret=igraph_pagerank(&g, &res, igraph_vss_all(), 1, 1000, 0.001, 1.2);
  if (ret != IGRAPH_EINVAL) {
    return 3;
  }
