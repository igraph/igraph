...
tmp <- tempdir()
dest <- paste(sep="", tmp, "/", "bkoff.dat")
download.file(url, dest)
l <- readLines(paste(sep="", tmp, "/bkoff.dat"))
...
g1$name <- "Bernard & Killworth office, BKOFFB"
g2$name <- "Bernard & Killworth office, BKOFFC"
...
bkoff <- list(BKOFFB=g1, BKOFFC=g2)
save(bkoff, file="/tmp/bkoff.Rdata.gz")
