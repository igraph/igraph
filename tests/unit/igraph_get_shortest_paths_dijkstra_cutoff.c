...
void checks_and_print(igraph_t *g, igraph_vector_int_list_t *vecs, igraph_vector_int_list_t *evecs,
        igraph_vector_int_t *parents, igraph_vector_int_t *inbound) {
    check_evecs(g, vecs, evecs, 20);
    check_parents_inbound(g, parents, inbound, /* from= */ 0, 50);

    for (igraph_integer_t i = 0; i < igraph_vector_int_list_size(vecs); i++) {
...
int main(void) {
    ...
    printf("Simple ring graph without weights:\n");
    ...
    printf("Same ring, same weights, cutoff 2:\n");
    igraph_get_shortest_paths_dijkstra_cutoff(&g, /*vertices=*/ &vecs,
                                       /*edges=*/ &evecs, /*from=*/ 0, /*to=*/ vs,
                                       &weights_vec, IGRAPH_OUT,
                                       &parents,
