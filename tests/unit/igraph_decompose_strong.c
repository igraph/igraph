...
int main() {
    ...
    igraph_write_graph_edgelist(igraph_graph_list_get_ptr(&complist, 0), stdout, /* onebased */ 0);
    igraph_destroy(&ring);
    ...
    for (i = 0; i < igraph_graph_list_size(&complist); i++) {
        igraph_write_graph_edgelist(igraph_graph_list_get_ptr(&complist, i), stdout, /* onebased */ 0);
    }
    igraph_destroy(&g);
