...
int main(void) {
    ...
    igraph_kary_tree(&graph, 10, 3, IGRAPH_TREE_UNDIRECTED);
    check_triangle_free("Tree", &graph);
    igraph_is_acyclic(&graph, &acyclic); /* populate cache */
    check_triangle_free("Tree", &graph);
    igraph_destroy(&graph);
    ...
    igraph_small(&graph, 4, IGRAPH_DIRECTED,
                 0,1, 1,2, 2,3, 3,0, 0,2,
                 -1);
    check_triangle_free("Directed without mutual, triangles, high edge count", &graph);
    igraph_is_simple(&graph, &simple, IGRAPH_UNDIRECTED); /* populate cache */
    check_triangle_free("Directed without mutual, triangles, high edge count", &graph);
    igraph_destroy(&graph);
