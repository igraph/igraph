...
void print_multiple(igraph_multiple_t multiple)
{
    switch (multiple)
    {
    case IGRAPH_MULTIPLE:
        printf("MULTIPLE;    ");
        break;
    case IGRAPH_NO_MULTIPLE:
        printf("NO_MULTIPLE; ");
        break;
    }
}
...
/* Print incidence list based on igraph_i_incident() */
...
void verify_ordering1(const igraph_t *graph, igraph_neimode_t mode, igraph_loops_t loops)
{
    ...
    for (igraph_integer_t v = 0; v < vcount; v++)
    {
        ...
        {
            igraph_integer_t u1, u2;

            u1 = VECTOR(neis)[i];
            switch (mode)
            {
            case IGRAPH_ALL:
                u2 = IGRAPH_OTHER(graph, VECTOR(incs)[i], v);
                break;
            case IGRAPH_OUT:
                u2 = IGRAPH_TO(graph, VECTOR(incs)[i]);
                break;
            case IGRAPH_IN:
                u2 = IGRAPH_FROM(graph, VECTOR(incs)[i]);
                break;
            }
