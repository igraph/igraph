...
int main(void) {
    ...
    igraph_diameter_bound(&g, &result, 0, NULL, NULL, IGRAPH_UNDIRECTED, 0);
    IGRAPH_ASSERT(result == 12);
    igraph_destroy(&g);

    // random barabasi - compare to default diameter
    igraph_rng_seed(igraph_rng_default(), 1234);
    igraph_barabasi_game(&g, 100, 1, 30, 0, 0, 1, IGRAPH_UNDIRECTED, IGRAPH_BARABASI_BAG, NULL);
    igraph_diameter_bound(&g, &result, 0, NULL, NULL, IGRAPH_UNDIRECTED, 0);
    igraph_diameter(&g, &reference, NULL, NULL, NULL, NULL, IGRAPH_UNDIRECTED, 1);
