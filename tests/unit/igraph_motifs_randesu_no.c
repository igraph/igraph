...
int main(void) {
    ...
    printf("No vertices:\n");
    call_and_print(&g_0, /*size*/ 3, NULL);

    printf("One vertex:\n");
    call_and_print(&g_1, /*size*/ 3, NULL);

    printf("Full graph of 50 vertices, motif size 3 (50 choose 3 = 19600):\n");
    call_and_print(&g_50_full, /*size*/ 3, NULL);
    ...
    call_and_print(&g_50_full, /*size*/ 4, NULL);

    printf("Triangle and a vertex, motif size 4:\n");
    call_and_print(&g_4_3_1, /*size*/ 4, NULL);

    VERIFY_FINALLY_STACK();

    printf("Cut prob too short.\n");
    CHECK_ERROR(igraph_motifs_randesu_no(&g_4_3_1, &result, /*size*/ 14, &cut_prob_01), IGRAPH_EINVAL);
