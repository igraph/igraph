...
void call_and_print(igraph_t *graph, igraph_vector_t *alpha, igraph_vector_t *alpham1) {
    ...
    IGRAPH_ASSERT(igraph_is_chordal(graph, alpha, alpham1, &chordal, &fill_in, &newgraph) == IGRAPH_SUCCESS);
    printf("Is chordal: %d\nFill in:\n", chordal);
...
int main() {
    ...
    printf("No vertices:\n");
    call_and_print(&g_0, NULL, NULL);

    printf("One vertex:\n");
    call_and_print(&g_1, NULL, NULL);

    printf("Disconnected graph with loops and multiple edges:\n");
    call_and_print(&g_lmu, NULL, NULL);
