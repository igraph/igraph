...
static void check_rewiring(igraph_tree_mode_t tree_mode, igraph_bool_t use_adjlist, igraph_bool_t allow_loops, const char* description) {
    ...
    igraph_degree(&g, &indegree_before, igraph_vss_all(), tree_mode == IGRAPH_TREE_UNDIRECTED ? IGRAPH_ALL : IGRAPH_IN, 1);
    igraph_degree(&g, &outdegree_before, igraph_vss_all(), tree_mode == IGRAPH_TREE_UNDIRECTED ? IGRAPH_ALL : IGRAPH_OUT, 1);
    ...
    igraph_degree(&g, &indegree_after, igraph_vss_all(), tree_mode == IGRAPH_TREE_UNDIRECTED ? IGRAPH_ALL : IGRAPH_IN, 1);
    igraph_degree(&g, &outdegree_after, igraph_vss_all(), tree_mode == IGRAPH_TREE_UNDIRECTED ? IGRAPH_ALL : IGRAPH_OUT, 1);
    ...
    if ((!igraph_vector_int_all_e(&indegree_before, &indegree_after)) ||
        ...
        printf("\nIn-degrees before and after:\n");
        print_vector_int(&indegree_before);
        print_vector_int(&indegree_after);

        printf("\nOut-degrees before and after:\n");
        print_vector_int(&outdegree_before);
        print_vector_int(&outdegree_after);
