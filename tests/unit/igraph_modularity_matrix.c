...
void test_print_destroy(igraph_t *g, igraph_vector_t *weights, float resolution, igraph_matrix_t *modmat) {
    IGRAPH_ASSERT(igraph_modularity_matrix(g, weights, resolution, modmat) == IGRAPH_SUCCESS);
    print_matrix(modmat);
    igraph_destroy(g);
...
int main() {
    ...
    test_print_destroy(&g, NULL, 1.0, &modmat);
    ...
    igraph_matrix_init(&modmat, 0, 0);
    test_print_destroy(&g, NULL, 0.0, &modmat);

    printf("Triangle and three points with self-loops:\n");
    igraph_small(&g, 6, /*directed*/0, 0,1, 0,2, 1,2, 3,3, 4,4, 5,5, -1);
    igraph_matrix_init(&modmat, 0, 0);
    test_print_destroy(&g, NULL, 1.0, &modmat);

    printf("Directed triangle:\n");
    ...
    igraph_matrix_init(&modmat, 0, 0);
    test_print_destroy(&g, &weights, 1.0, &modmat);
