...
void test_directed(void) {
    ...
    /* Acyclic graph */
    igraph_erdos_renyi_game_gnm(&graph, 20, 30, IGRAPH_DIRECTED, IGRAPH_NO_LOOPS);
    ...
    for (igraph_integer_t m=10; m <= 60; m += 10) {
        igraph_erdos_renyi_game_gnm(&graph, 20, m, IGRAPH_DIRECTED, IGRAPH_LOOPS);
        compare_methods(&graph, NULL);
        rand_weights(&graph, &weights);
        compare_methods(&graph, &weights);
        igraph_destroy(&graph);
    ...
    igraph_erdos_renyi_game_gnm(&graph, 200, 400, IGRAPH_DIRECTED, IGRAPH_LOOPS);
    igraph_feedback_arc_set(&graph, &result, NULL, IGRAPH_FAS_EXACT_IP_CG);
    igraph_destroy(&graph);
