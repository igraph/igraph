...
int igraph_is_eulerian(igraph_t *graph, igraph_bool_t *has_path, igraph_bool_t *has_cycle) {
    if (igraph_is_directed(graph)) {
        ...
        IGRAPH_CHECK(is_eulerian_undirected(graph, has_path, has_cycle));
    } 
    return IGRAPH_SUCCESS;
}

int check_if_bridge(igraph_integer_t start, igraph_integer_t v, long edge, igraph_bool_t *is_bridge, igraph_vector_t *bridges) {
...
int euler_undirected_implementation(igraph_integer_t start, igraph_t *g, igraph_vector_t *path, igraph_vector_bool_t *visited_list, igraph_vector_t *bridges) {
...
int igraph_euler_path_undirected(igraph_t *graph, igraph_vector_t *path) {
    ...
    igraph_vector_int_t *incedges;
    igraph_vector_t degree, bridges;
    ...
    if (cycle && !has_path) {
        ...
        euler_undirected_implementation(start, graph, path, &visited_list, &bridges);
    }
...
int igraph_eulerian_path_directed(igraph_t *graph, igraph_vector_t *res) {
    ...
    if ((outgoing_excess == 0 && incoming_excess == 0) && (res_strong)) {
        ...
        IGRAPH_CHECK(eulerian_path_directed_implementation(graph, &start_node, &out_degree, res));
        igraph_vector_destroy(&degree);
        IGRAPH_FINALLY_CLEAN(1);
    } else if ((outgoing_excess == 1 && incoming_excess == 1) && 
        (res_strong || res_weak)) {
...
int igraph_eulerian_cycle(igraph_t *graph, igraph_vector_t *res) {
    ...
    if (igraph_is_directed(graph)) {
        IGRAPH_CHECK(igraph_eulerian_path_directed(graph, res));
    } else {
        IGRAPH_CHECK(igraph_euler_path_undirected(graph, res));
    }
