...
igraph_long_t igraph_running_mean(const igraph_vector_t *data, igraph_vector_t *res,
    ...
    IGRAPH_CHECK(igraph_vector_resize(res, (igraph_long_t)(igraph_vector_size(data) - binwidth + 1)));
    ...
    for (i = 1; i < igraph_vector_size(data) - binwidth + 1; i++) {
        ...
        sum -= VECTOR(*data)[i - 1];
        sum += VECTOR(*data)[ (igraph_long_t)(i + binwidth - 1)];
 ...
 */
igraph_long_t igraph_convex_hull(const igraph_matrix_t *data, igraph_vector_t *resverts,
                       igraph_matrix_t *rescoords) {
    igraph_long_t no_of_nodes;
    igraph_long_t i, pivot_idx = 0, last_idx, before_last_idx, next_idx, j;
    igraph_vector_t angles, stack, order;
    igraph_real_t px, py, cp;

    no_of_nodes = (igraph_long_t) igraph_matrix_nrow(data);
    ...
    j = 0;
    last_idx = (igraph_long_t) VECTOR(order)[0];
    pivot_idx = (igraph_long_t) VECTOR(order)[no_of_nodes - 1];
    for (i = 1; i < no_of_nodes; i++) {
        next_idx = (igraph_long_t) VECTOR(order)[i];
    ...
    while (!igraph_vector_empty(&order)) {
        next_idx = (igraph_long_t)VECTOR(order)[igraph_vector_size(&order) - 1];
        if (next_idx < 0) {
        ...
        for (igraph_long_t k=0; k<j; k++) printf("%ld ", (igraph_long_t)VECTOR(stack)[k]);
        printf("]\n");
        */
        if (cp < 0) {
            ...
            before_last_idx = (j >= 2) ? (igraph_long_t) VECTOR(stack)[j - 2] : -1;
        }
    }
 ...
 */
igraph_long_t igraph_power_law_fit(const igraph_vector_t* data, igraph_plfit_result_t* result,
    ...
    if (discrete) {
        ...
        for (i = 0; i < n; i++) {
            if ((igraph_long_t)(VECTOR(*data)[i]) != VECTOR(*data)[i]) {
                discrete = 0;
                break;
            }
        }
