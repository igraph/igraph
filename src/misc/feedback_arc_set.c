...
igraph_long_t igraph_i_feedback_arc_set_undirected(const igraph_t *graph, igraph_vector_t *result,
    ...
    if (layering != 0) {
        ...
        IGRAPH_CHECK((igraph_long_t) igraph_vector_qsort_ind(&degrees, &roots,
                     /* descending = */ 1));
...
igraph_long_t igraph_i_feedback_arc_set_eades(const igraph_t *graph, igraph_vector_t *result,
    ...
    igraph_long_t* ordering;
    igraph_long_t order_next_pos = 0, order_next_neg = -1;
    igraph_real_t diff, maxdiff;

    ordering = igraph_Calloc(no_of_nodes, igraph_long_t);
    ...
    while (nodes_left > 0) {
        ...
        while (!igraph_dqueue_empty(&sources)) {
            i = (igraph_long_t)igraph_dqueue_pop(&sources);
            ...
            for (i = 0; i < j; i++) {
                eid = (igraph_long_t) VECTOR(neis)[i];
        ...
        while (!igraph_dqueue_empty(&sinks)) {
            i = (igraph_long_t)igraph_dqueue_pop(&sinks);
        ...
        if (v >= 0) {
            ...
            IGRAPH_CHECK(igraph_incident(graph, &neis, (igraph_long_t) v,
                                         IGRAPH_OUT));
            j = igraph_vector_size(&neis);
            for (i = 0; i < j; i++) {
                eid = (igraph_long_t) VECTOR(neis)[i];
    ...
    if (result != 0) {
        ...
        for (i = 0; i < j; i++) {
            igraph_long_t from = IGRAPH_FROM(graph, i), to = IGRAPH_TO(graph, i);
    ...
    if (layers != 0) {
        ...
        IGRAPH_CHECK((igraph_long_t) igraph_vector_long_qsort_ind(&order_vec, &ranks, 0));

        for (i = 0; i < no_of_nodes; i++) {
            igraph_long_t from = (igraph_long_t) VECTOR(ranks)[i];
            IGRAPH_CHECK(igraph_neighbors(graph, &neis, (igraph_long_t) from,
                                          IGRAPH_OUT));
            k = igraph_vector_size(&neis);
            for (j = 0; j < k; j++) {
                igraph_long_t to = (igraph_long_t) VECTOR(neis)[j];
                if (from == to) {
                    continue;
                }
...
igraph_long_t igraph_i_feedback_arc_set_ip(const igraph_t *graph, igraph_vector_t *result,
    ...
    igraph_long_t no_of_components;
    ...
    igraph_long_t i, j, k, l, m, n, from, to;
    ...
    for (i = 0; i < no_of_vertices; i++) {
        j = (igraph_long_t) VECTOR(membership)[i];
        IGRAPH_CHECK(igraph_vector_push_back(VECTOR(vertices_by_components)[j], i));
    }
    for (i = 0; i < no_of_edges; i++) {
        j = (igraph_long_t) VECTOR(membership)[(igraph_long_t)IGRAPH_FROM(graph, i)];
...
#define VAR2IDX(i, j) (i*(n-1)+j-(i+1)*i/2)
    ...
    for (i = 0; i < no_of_components; i++) {
        ...
        for (j = 0; j < n; j++) {
            VECTOR(vertex_remapping)[(igraph_long_t)VECTOR(*vertices_in_comp)[j]] = j;
        }

        /* Set up variables */
        k = n * (n - 1) / 2;
        if (k > 0) {
            glp_add_cols(ip, (igraph_long_t) k);
            for (j = 1; j <= k; j++) {
                glp_set_col_kind(ip, (igraph_long_t) j, GLP_BV);
            }
        }
        ...
        for (j = 0; j < k; j++) {
            l = (igraph_long_t) VECTOR(*edges_in_comp)[j];
            from = (igraph_long_t) VECTOR(vertex_remapping)[(igraph_long_t)IGRAPH_FROM(graph, l)];
            to = (igraph_long_t) VECTOR(vertex_remapping)[(igraph_long_t)IGRAPH_TO(graph, l)];
            if (from == to) {
                continue;
            }
            ...
            if (from < to) {
                l = VAR2IDX(from, to);
                glp_set_obj_coef(ip, (igraph_long_t) l, glp_get_obj_coef(ip, (igraph_long_t) l) + weight);
            } else {
                l = VAR2IDX(to, from);
                glp_set_obj_coef(ip, (igraph_long_t) l, glp_get_obj_coef(ip, (igraph_long_t) l) - weight);
            }
        ...
        if (n > 1) {
            ...
            for (j = 0; j < n; j++) {
                ...
                for (k = j + 1; k < n; k++) {
                    ...
                    val[2] = 1;
                    for (l = k + 1; l < n; l++, m++) {
                        ind[2] = (igraph_long_t) VAR2IDX(k, l);
                        ind[3] = (igraph_long_t) VAR2IDX(j, l);
                        glp_set_row_bnds(ip, (igraph_long_t) m, GLP_UP, 1, 1);
                        glp_set_mat_row(ip, (igraph_long_t) m, 3, ind, val);
                    }
                    /* Type (2b) */
                    val[2] = -1;
                    for (l = j + 1; l < k; l++, m++) {
                        ind[2] = (igraph_long_t) VAR2IDX(l, k);
                        ind[3] = (igraph_long_t) VAR2IDX(j, l);
                        glp_set_row_bnds(ip, (igraph_long_t) m, GLP_UP, 0, 0);
                        glp_set_mat_row(ip, (igraph_long_t) m, 3, ind, val);
                    }
                }
            }
        ...
        for (l = 1; l <= m; l++) {
            ...
            if (glp_mip_col_val(ip, (igraph_long_t) l) > 0) {
