...
igraph_long_t igraph_maximum_cardinality_search(const igraph_t *graph,
    ...
    while (i >= 1) {
        ...
        len = igraph_vector_long_size(neis);
        for (k = 0; k < len; k++) {
            igraph_long_t w = (igraph_long_t) VECTOR(*neis)[k];
            igraph_long_t ws = VECTOR(size)[w];
            if (ws >= 0) {
                ...
                igraph_long_t nw = VECTOR(next)[w];
                igraph_long_t pw = VECTOR(prev)[w];
                if (nw != 0) {
                    VECTOR(prev)[nw - 1] = pw;
                }
                if (pw != 0) {
...
igraph_long_t igraph_is_chordal(const igraph_t *graph,
    ...
    if (!alpha && !alpham1) {
        ...
        my_alpham1 = &v_alpham1;
        for (v = 0; v < no_of_nodes; v++) {
            igraph_long_t i = (igraph_long_t) VECTOR(*my_alpha)[v];
        ...
        for (i = 0; i < no_of_nodes; i++) {
            igraph_long_t v = (igraph_long_t) VECTOR(*my_alpham1)[i];
            VECTOR(*my_alpha)[v] = i;
        }
    }
    ...
    for (i = 0; i < no_of_nodes; i++) {
        ...
        w = (igraph_long_t) VECTOR(*my_alpham1)[i];
        VECTOR(f)[w] = w;
        VECTOR(index)[w] = i;
