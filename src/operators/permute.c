 ...
 * vertices and edges.
 */
igraph_long_t igraph_permute_vertices(const igraph_t *graph, igraph_t *res,
    ...
    for (i = 0; i < no_of_edges; i++) {
        VECTOR(edges)[p++] = VECTOR(*permutation)[ (igraph_long_t) IGRAPH_FROM(graph, i) ];
        VECTOR(edges)[p++] = VECTOR(*permutation)[ (igraph_long_t) IGRAPH_TO(graph, i) ];
    }
    ...
    if (graph->attr) {
        ...
        if (igraph_vector_size(&vtypes) != 0) {
            ...
            for (i = 0; i < no_of_nodes; i++) {
                VECTOR(index)[ (igraph_long_t) VECTOR(*permutation)[i] ] = i;
            }
