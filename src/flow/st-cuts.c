...
igraph_error_t igraph_even_tarjan_reduction(const igraph_t *graph, igraph_t *graphbar,
    ...
    igraph_long_t new_no_of_nodes = no_of_nodes * 2;
    igraph_long_t new_no_of_edges = no_of_nodes + no_of_edges * 2;

    igraph_vector_t edges;
    igraph_long_t edgeptr = 0, capptr = 0;
    igraph_long_t i;
    ...
    IGRAPH_CHECK(igraph_create(graphbar, &edges, (igraph_long_t)
...
igraph_error_t igraph_i_residual_graph(const igraph_t *graph,
    ...
    igraph_long_t no_of_nodes = igraph_vcount(graph);
    igraph_long_t no_of_edges = igraph_ecount(graph);
    igraph_long_t i, no_new_edges = 0;
    igraph_long_t edgeptr = 0, capptr = 0;
    ...
    IGRAPH_CHECK(igraph_create(residual, tmp, (igraph_long_t) no_of_nodes,
                               IGRAPH_DIRECTED));
...
static igraph_long_t igraph_i_dbucket_init(igraph_i_dbucket_t *buck, igraph_long_t size) {
...
static igraph_long_t igraph_i_dbucket_insert(igraph_i_dbucket_t *buck, igraph_long_t bid,
                                   igraph_long_t elem) {
...
static igraph_long_t igraph_i_dbucket_empty(const igraph_i_dbucket_t *buck,
                                       igraph_long_t bid) {
    return VECTOR(buck->head)[bid] == 0;
}

static igraph_long_t igraph_i_dbucket_delete(igraph_i_dbucket_t *buck, igraph_long_t bid) {
    igraph_long_t elem = VECTOR(buck->head)[bid] - 1;
    VECTOR(buck->head)[bid] = VECTOR(buck->next)[elem];
    return elem;
}

static igraph_long_t igraph_i_dominator_LINK(igraph_long_t v, igraph_long_t w,
...
static igraph_long_t igraph_i_dominator_COMPRESS(igraph_long_t v,
                                       igraph_vector_long_t *ancestor,
                                       igraph_vector_long_t *label,
                                       igraph_vector_long_t *semi) {
    igraph_stack_long_t path;
    igraph_long_t w = v;
    igraph_long_t top, pretop;
...
static igraph_long_t igraph_i_dominator_EVAL(igraph_long_t v,
                                        igraph_vector_long_t *ancestor,
                                        igraph_vector_long_t *label,
                                        igraph_vector_long_t *semi) {
...
igraph_error_t igraph_dominator_tree(const igraph_t *graph,
    ...
    igraph_long_t component_size = 0;
    ...
    for (i = 0; i < no_of_nodes; i++) {
        if (IGRAPH_FINITE(VECTOR(vertex)[i])) {
            igraph_long_t t = (igraph_long_t) VECTOR(vertex)[i];
    ...
    if (leftout) {
        igraph_long_t n = no_of_nodes - component_size;
        igraph_long_t p = 0, j;
    ...
    for (i = 0; i < no_of_nodes; i++) {
        igraph_vector_long_t *v = igraph_adjlist_get(&pred, i);
        igraph_long_t j, n = igraph_vector_long_size(v);
        for (j = 0; j < n; ) {
            igraph_long_t v2 = (igraph_long_t) VECTOR(*v)[j];
            if (IGRAPH_FINITE(VECTOR(parent)[v2])) {
                j++;
            } else {
                VECTOR(*v)[j] = VECTOR(*v)[n - 1];
                igraph_vector_long_pop_back(v);
                n--;
            }
        }
    }
    ...
    for (i = component_size - 1; i > 0; i--) {
        igraph_long_t w = (igraph_long_t) VECTOR(vertex)[i] - 1;
        igraph_vector_long_t *predw = igraph_adjlist_get(&pred, w);
        igraph_long_t j, n = igraph_vector_long_size(predw);
        for (j = 0; j < n; j++) {
            igraph_long_t v = (igraph_long_t) VECTOR(*predw)[j];
            igraph_long_t u = igraph_i_dominator_EVAL(v, &ancestor, &label, &semi);
            if (VECTOR(semi)[u] < VECTOR(semi)[w]) {
                VECTOR(semi)[w] = VECTOR(semi)[u];
            }
        }
        igraph_i_dbucket_insert(&bucket, (igraph_long_t)
                                VECTOR(vertex)[ VECTOR(semi)[w] - 1 ] - 1, w);
        igraph_i_dominator_LINK((igraph_long_t) VECTOR(parent)[w], w, &ancestor);
        while (!igraph_i_dbucket_empty(&bucket, (igraph_long_t) VECTOR(parent)[w])) {
            igraph_long_t v = igraph_i_dbucket_delete(&bucket, (igraph_long_t) VECTOR(parent)[w]);
            igraph_long_t u = igraph_i_dominator_EVAL(v, &ancestor, &label, &semi);
    ...
    for (i = 1; i < component_size; i++) {
        ...
        if (VECTOR(*mydom)[w] != VECTOR(vertex)[VECTOR(semi)[w] - 1] - 1) {
            VECTOR(*mydom)[w] = VECTOR(*mydom)[(igraph_long_t)VECTOR(*mydom)[w]];
        }
    }
    VECTOR(*mydom)[(igraph_long_t)root] = -1;
    ...
    if (domtree) {
        ...
        IGRAPH_CHECK(igraph_create(domtree, &edges, (igraph_long_t) no_of_nodes,
                                   IGRAPH_DIRECTED));
        igraph_vector_destroy(&edges);
        IGRAPH_FINALLY_CLEAN(1);
...
static igraph_bool_t igraph_i_all_st_cuts_minimal_dfs_incb(
    ...
    igraph_long_t realvid = (igraph_long_t) VECTOR(*map)[(igraph_long_t)vid];

    IGRAPH_UNUSED(graph); IGRAPH_UNUSED(dist);

    if (VECTOR(*GammaX)[(igraph_long_t)realvid]) {
        if (!igraph_stack_empty(stack)) {
            igraph_long_t top = (igraph_long_t) igraph_stack_top(stack);
...
static igraph_long_t igraph_i_all_st_cuts_minimal(const igraph_t *graph,
    ...
    IGRAPH_CHECK(igraph_dfs(domtree, (igraph_long_t) root, IGRAPH_IN,
...
igraph_error_t igraph_i_all_st_cuts_pivot(const igraph_t *graph,
    ...
    root = (igraph_long_t) VECTOR(Sbar_map)[target] - 1;

    /* -------------------------------------------------------------*/
    /* Construct the dominator tree of Sbar */

    IGRAPH_VECTOR_INIT_FINALLY(&leftout, 0);
    IGRAPH_CHECK(igraph_dominator_tree(&Sbar, (igraph_long_t) root,
                                       /*dom=*/ 0, &domtree,
                                       &leftout, IGRAPH_IN));
    ...
    if (igraph_marked_queue_size(S) == 0) {
        VECTOR(GammaS)[(igraph_long_t) VECTOR(Sbar_map)[source] - 1] = 1;
    } else {
        for (i = 0; i < no_of_nodes; i++) {
            if (igraph_marked_queue_iselement(S, i)) {
    ...
    for (i = 0; i < n; i++) {
        VECTOR(leftout)[i] = VECTOR(Sbar_invmap)[(igraph_long_t)VECTOR(leftout)[i]];
        VECTOR(GammaS)[(igraph_long_t)VECTOR(leftout)[i]] = 0;
    }
    ...
    for (i = 0; i < nomin; i++) {
        ...
        igraph_long_t min = (igraph_long_t) VECTOR(Sbar_map)[(igraph_long_t) VECTOR(M)[i] ] - 1;
        igraph_long_t nuvsize, isvlen, j;
        IGRAPH_CHECK(igraph_dfs(&domtree, (igraph_long_t) min, IGRAPH_IN,
        ...
        for (nuvsize = 0; nuvsize < Sbar_size; nuvsize++) {
            ...
            if (IGRAPH_FINITE(t)) {
                VECTOR(Nuv)[nuvsize] = VECTOR(Sbar_invmap)[(igraph_long_t) t];
            } else {
                break;
            }
        }
        ...
        for (j = 0; j < isvlen; j++) {
            igraph_long_t u = (igraph_long_t) VECTOR(Isv_min)[j];
        ...
        if (j == isvlen) {
            *v = (igraph_long_t) VECTOR(M)[i];
            /* Calculate real Isv */
            IGRAPH_CHECK(igraph_vector_append(&Nuv, &leftout));
            IGRAPH_CHECK(igraph_bfs(graph, /*root=*/ (igraph_long_t) *v,
...
igraph_error_t igraph_provan_shier_list(const igraph_t *graph,
    ...
    if (igraph_vector_size(&Isv) == 0) {
        ...
        for (i = 0; i < n; i++) {
            if (!igraph_marked_queue_iselement(S, (igraph_long_t) VECTOR(Isv)[i])) {
                igraph_marked_queue_push(S, (igraph_long_t) VECTOR(Isv)[i]);
            }
        }
    ...
    */
    ...
    if (cuts) {
        ...
        for (i = 0; i < nocuts; i++) {
            ...
            igraph_long_t cutsize = 0;
            igraph_long_t j, partlen = igraph_vector_size(part);
            /* Mark elements */
            for (j = 0; j < partlen; j++) {
                igraph_long_t v = (igraph_long_t) VECTOR(*part)[j];
                VECTOR(inS)[v] = i + 1;
            }
            /* Check how many edges */
            for (j = 0; j < no_of_edges; j++) {
                igraph_long_t from = IGRAPH_FROM(graph, j);
                igraph_long_t to = IGRAPH_TO(graph, j);
                igraph_long_t pfrom = VECTOR(inS)[from];
                igraph_long_t pto = VECTOR(inS)[to];
...
static igraph_long_t igraph_i_all_st_mincuts_minimal(const igraph_t *Sbar,
                                           const igraph_vector_bool_t *active,
                                           const igraph_vector_t *invmap,
                                           igraph_vector_t *minimal) {

    igraph_long_t no_of_nodes = igraph_vcount(Sbar);
    igraph_vector_t indeg;
    igraph_long_t i, minsize;
    igraph_vector_t neis;
...
#define ACTIVE(x) (VECTOR(*active)[(igraph_long_t)VECTOR(*invmap)[(x)]])
#define ZEROIN(x) (VECTOR(indeg)[(x)]==0)

    for (i = 0; i < no_of_nodes; i++) {
        if (!ACTIVE(i)) {
            igraph_long_t j, n;
            IGRAPH_CHECK(igraph_neighbors(Sbar, &neis, (igraph_long_t) i,
                                          IGRAPH_OUT));
            n = igraph_vector_size(&neis);
            for (j = 0; j < n; j++) {
                igraph_long_t nei = (igraph_long_t) VECTOR(neis)[j];
...
static igraph_error_t igraph_i_all_st_mincuts_pivot(const igraph_t *graph,
    ...
    nomin = igraph_vector_size(&M);
    for (i = 0; i < nomin; i++) {
        igraph_long_t min = (igraph_long_t) VECTOR(Sbar_invmap)[ (igraph_long_t) VECTOR(M)[i] ];
        if (min != target)
    ...
    if (i != nomin) {
        ...
        igraph_vector_t Isv_min;
        IGRAPH_VECTOR_INIT_FINALLY(&Isv_min, 0);
        *v = (igraph_long_t) VECTOR(Sbar_invmap)[ (igraph_long_t) VECTOR(M)[i] ];
        /* TODO: restricted == keep ? */
        IGRAPH_CHECK(igraph_bfs(graph, /*root=*/ (igraph_long_t) *v,/*roots=*/ 0,
...
igraph_error_t igraph_all_st_mincuts(const igraph_t *graph, igraph_real_t *value,
    ...
    igraph_long_t newsource, newtarget;
    igraph_marked_queue_t S;
    igraph_estack_t T;
    igraph_i_all_st_mincuts_data_t pivot_data;
    igraph_vector_bool_t VE1bool;
    igraph_vector_t VE1;
    igraph_long_t VE1size = 0;
    igraph_long_t i, nocuts;
    igraph_long_t proj_nodes;
    ...
    newsource = (igraph_long_t) VECTOR(NtoL)[(igraph_long_t)source];
    newtarget = (igraph_long_t) VECTOR(NtoL)[(igraph_long_t)target];
    ...
    for (i = 0; i < no_of_edges; i++) {
        if (VECTOR(flow)[i] > 0) {
            igraph_long_t from = IGRAPH_FROM(graph, i);
            igraph_long_t to = IGRAPH_TO(graph, i);
            igraph_long_t pfrom = (igraph_long_t) VECTOR(NtoL)[from];
            igraph_long_t pto = (igraph_long_t) VECTOR(NtoL)[to];
    ...
    for (i = 0; i < no_of_nodes; i++) {
        igraph_long_t id = (igraph_long_t) VECTOR(NtoL)[i];
    ...
    for (i = 0; i < nocuts; i++) {
        ...
        igraph_long_t j, supercutsize = igraph_vector_size(supercut);
        ...
        for (j = 0; j < supercutsize; j++) {
            igraph_long_t vtx = (igraph_long_t) VECTOR(*supercut)[j];
            igraph_long_t ovtx = (igraph_long_t) VECTOR(revmap_ptr)[vtx];
            while (ovtx != 0) {
                ovtx--;
                IGRAPH_CHECK(igraph_vector_push_back(cut, ovtx));
                ovtx = (igraph_long_t) VECTOR(revmap_next)[ovtx];
            }
        }
    ...
    if (cuts) {
        ...
        for (i = 0; i < nocuts; i++) {
            igraph_vector_t *part = VECTOR(*mypartition1s)[i];
            igraph_long_t j, n = igraph_vector_size(part);
            igraph_vector_t *v;
            ...
            for (j = 0; j < n; j++) {
                igraph_long_t vtx = (igraph_long_t) VECTOR(*part)[j];
                VECTOR(memb)[vtx] = i + 1;
            }
            for (j = 0; j < no_of_edges; j++) {
                if (VECTOR(flow)[j] > 0) {
