...
c     %----------------------%
c
      external   IGRAPHARPACKDAXPY, IGRAPHARPACKDCOPY, IGRAPHARPACKDSCAL, IGRAPHARPACKDLACPY, IGRAPHARPACKDLARTG, IGRAPHARPACKDLASET, IGRAPHARPACKDVOUT, 
     &           IGRAPHARPACKIVOUT, IGRAPHARPACKSECOND, IGRAPHARPACKDGEMV
c
c     %--------------------%
...
c
      if (first) then
         epsmch = IGRAPHARPACKDLAMCH('Epsilon-Machine')
         first = .false.
      end if
      itop = 1
c
...
c
     ...
     &                 '_sapps: occured before shift number.')
                  call IGRAPHARPACKDVOUT (logfil, 1, h(i+1,1), ndigit, 
...
c
         if (h(iend,1) .lt. zero) then
             h(iend,1) = -h(iend,1)
             call IGRAPHARPACKDSCAL(kplusp, -one, q(1,iend), 1)
         end if
c
c        %--------------------------------------------------------%
...
c
     ...
     &   call IGRAPHARPACKDGEMV ('N', n, kplusp, one, v, ldv,
...
c     %-------------------------------------------------%
c
      call IGRAPHARPACKDLACPY ('All', n, kev, v(1,np+1), ldv, v, ldv)
c 
c     %--------------------------------------------%
...
c
      ...
      if (h(kev+1,1) .gt. zero) 
     &   call IGRAPHARPACKDAXPY (n, h(kev+1,1), v(1,kev+1), 1, resid, 1)
c
      if (msglvl .gt. 1) then
         call IGRAPHARPACKDVOUT (logfil, 1, q(kplusp,kev), ndigit, 
     &      '_sapps: sigmak of the updated residual vector')
         call IGRAPHARPACKDVOUT (logfil, 1, h(kev+1,1), ndigit, 
     &      '_sapps: betak of the updated residual vector')
         call IGRAPHARPACKDVOUT (logfil, kev, h(1,2), ndigit, 
     &      '_sapps: updated main diagonal of H for next iteration')
         if (kev .gt. 1) then
         call IGRAPHARPACKDVOUT (logfil, kev-1, h(2,1), ndigit, 
