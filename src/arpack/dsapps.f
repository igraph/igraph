...
c     %----------------------%
c
      external   igraphdaxpy, igraphdcopy, igraphdscal, igraphdlacpy, 
     &           igraphdlartg, igraphdlaset, igraphdvout, 
     &           igraphivout, igraphsecond, igraphdgemv
c
c     %--------------------%
...
c
      if (first) then
         epsmch = igraphdlamch('Epsilon-Machine')
         first = .false.
      end if
      itop = 1
c
...
c     %----------------------------------------------%
c
      call igraphdlaset ('All', kplusp, kplusp, zero, one, q, ldq)
c
c     %----------------------------------------------%
...
c
         if (h(iend,1) .lt. zero) then
             h(iend,1) = -h(iend,1)
             call igraphdscal(kplusp, -one, q(1,iend), 1)
         end if
c
c        %--------------------------------------------------------%
...
c
     ...
     &   call igraphdgemv ('N', n, kplusp, one, v, ldv,
...
c
      do 130 i = 1, kev
         call igraphdgemv ('N', n, kplusp-i+1, one, v, ldv,
     &               q(1,kev-i+1), 1, zero, workd, 1)
         call igraphdcopy (n, workd, 1, v(1,kplusp-i+1), 1)
  130 continue
c
c     %-------------------------------------------------%
...
c     %-------------------------------------------------%
c
      call igraphdlacpy ('All', n, kev, v(1,np+1), ldv, v, ldv)
c 
c     %--------------------------------------------%
...
c
      if ( h(kev+1,1) .gt. zero ) 
     &     call igraphdcopy (n, workd(n+1), 1, v(1,kev+1), 1)
c 
c     %-------------------------------------%
...
c     %-------------------------------------%
c
      call igraphdscal (n, q(kplusp,kev), resid, 1)
      if (h(kev+1,1) .gt. zero) 
     &   call igraphdaxpy (n, h(kev+1,1), v(1,kev+1), 1, resid, 1)
c
      if (msglvl .gt. 1) then
