...
c\BeginDoc
c
c\Name: IGRAPHARPACKDSAUP2
c
c\Description: 
c  Intermediate level interface called by IGRAPHARPACKDSAUPD.
c
c\Usage:
c  call IGRAPHARPACKDSAUP2 
...
c\Arguments
c
c  IDO, BMAT, N, WHICH, NEV, TOL, RESID: same as defined in IGRAPHARPACKDSAUPD.
c  MODE, ISHIFT, MXITER: see the definition of IPARAM in IGRAPHARPACKDSAUPD.
c  
...
c          at H(2,1).  The main diagonal is stored in the IGRAPHARPACKSECOND column
c          of H starting at H(1,2). If IGRAPHARPACKDSAUP2 converges store the 
...
c          See Data Distribution Note in IGRAPHARPACKDSAUPD.
c
...
c     IGRAPHARPACKDSAITR  ARPACK Lanczos factorization routine.
c     IGRAPHARPACKDSAPPS  ARPACK application of implicit shifts routine.
c     IGRAPHARPACKDSCONV  ARPACK convergence of Ritz values routine.
c     IGRAPHARPACKDSEIGT  ARPACK compute Ritz values and error bounds routine.
c     IGRAPHARPACKDSGETS  ARPACK reorder Ritz values and error bounds routine.
c     IGRAPHARPACKDSORTR  ARPACK sorting routine.
...
c     %----------------------%
c
      external   IGRAPHARPACKDCOPY, IGRAPHARPACKDGETV0, IGRAPHARPACKDSAITR, IGRAPHARPACKDSCAL, IGRAPHARPACKDSCONV, IGRAPHARPACKDSEIGT, IGRAPHARPACKDSGETS, 
     &           IGRAPHARPACKDSAPPS, IGRAPHARPACKDSORTR, IGRAPHARPACKDVOUT, IGRAPHARPACKIVOUT, IGRAPHARPACKSECOND, IGRAPHARPACKDSWAP
c
c     %--------------------%
...
c
      Double precision
     &           IGRAPHARPACKDDOT, IGRAPHARPACKDNRM2, IGRAPHARPACKDLAMCH
      external   IGRAPHARPACKDDOT, IGRAPHARPACKDNRM2, IGRAPHARPACKDLAMCH
c
c     %---------------------%
...
c     %----------------------------------------------------------%
c
      call IGRAPHARPACKDSAITR (ido, bmat, n, 0, nev0, mode, resid, rnorm, v, ldv, 
...
c
c        %-----------------------------------------------------%
c        | IGRAPHARPACKDSAITR was unable to build an Lanczos factorization |
...
c
   ...
   20    continue
         update = .true.
c
         call IGRAPHARPACKDSAITR (ido, bmat, n, nev, np, mode, resid, rnorm, v, 
...
c        %--------------------------------------------------------%
c
         call IGRAPHARPACKDSEIGT (rnorm, kplusp, h, ldh, ritz, bounds, workl, ierr)
c
         if (ierr .ne. 0) then
            info = -8
            go to 1200
...
c        %----------------------------------------------------%
c
         call IGRAPHARPACKDCOPY(kplusp, ritz, 1, workl(kplusp+1), 1)
         call IGRAPHARPACKDCOPY(kplusp, bounds, 1, workl(2*kplusp+1), 1)
c
c        %---------------------------------------------------%
...
c
         nev = nev0
         np = np0
         call IGRAPHARPACKDSGETS (ishift, which, nev, np, ritz, bounds, workl)
c 
c        %-------------------%
c        | Convergence test. |
c        %-------------------%
c
         call IGRAPHARPACKDCOPY (nev, bounds(np+1), 1, workl(np+1), 1)
         call IGRAPHARPACKDSCONV (nev, ritz(np+1), workl(np+1), tol, nconv)
c
         if (msglvl .gt. 2) then
            kp(1) = nev
            kp(2) = np
            kp(3) = nconv
            call IGRAPHARPACKIVOUT (logfil, 3, kp, ndigit,
     &                  '_saup2: NEV, NP, NCONV are')
            call IGRAPHARPACKDVOUT (logfil, kplusp, ritz, ndigit,
...
c
               wprime = 'SA'
               call IGRAPHARPACKDSORTR (wprime, .true., kplusp, ritz, bounds)
               nevd2 = nev / 2
               nevm2 = nev - nevd2 
               if ( nev .gt. 1 ) then
                  call IGRAPHARPACKDSWAP ( min(nevd2,np), ritz(nevm2+1), 1,
     &                 ritz( max(kplusp-nevd2+1,kplusp-np+1) ), 1)
                  call IGRAPHARPACKDSWAP ( min(nevd2,np), bounds(nevm2+1), 1,
...
c
            wprime = 'LA'
            call IGRAPHARPACKDSORTR(wprime, .true., nev0, bounds, ritz)
c
c           %----------------------------------------------%
...
c
               wprime = 'LA'
               call IGRAPHARPACKDSORTR(wprime, .true., nconv, ritz, bounds)
c
            else
c
c              %----------------------------------------------%
...
c              %----------------------------------------------%
               ...
               call IGRAPHARPACKDSORTR(which, .true., nconv, ritz, bounds)
c
            end if
c
c           %------------------------------------------%
...
c     
            if (nevbef .lt. nev) 
     &         call IGRAPHARPACKDSGETS (ishift, which, nev, np, ritz, bounds,
     &              workl)
c
         end if
c
     ...
     &              '_saup2: NEV and NP are')
               call IGRAPHARPACKDVOUT (logfil, nev, ritz(np+1), ndigit,
...
c        | in the exact shift case, IGRAPHARPACKDSGETS already handles this.   |
c        %---------------------------------------------------------%
c
         if (ishift .eq. 0) call IGRAPHARPACKDCOPY (np, workl, 1, ritz, 1)
c
         if (msglvl .gt. 2) then
            call IGRAPHARPACKIVOUT (logfil, 1, np, ndigit,
     &                  '_saup2: The number of shifts to apply ')
            call IGRAPHARPACKDVOUT (logfil, np, workl, ndigit,
...
c        | After IGRAPHARPACKDSAPPS is done, we have a Lanczos                 |
c        | factorization of length NEV.                            |
c        %---------------------------------------------------------%
c
         call IGRAPHARPACKDSAPPS (n, nev, np, ritz, v, ldv, h, ldh, resid, q, ldq,
     &        workd)
c
c        %---------------------------------------------%
c        | Compute the B-norm of the updated residual. |
c        | Keep B*RESID in WORKD(1:N) to be used in    |
c        | the first step of the next call to IGRAPHARPACKDSAITR.  |
c        %---------------------------------------------%
c
         cnorm = .true.
...
c
     ...
     &      '_saup2: B-norm of residual for NEV factorization')
            call IGRAPHARPACKDVOUT (logfil, nev, h(1,2), ndigit,
     &           '_saup2: main diagonal of compressed H matrix')
            call IGRAPHARPACKDVOUT (logfil, nev-1, h(2,1), ndigit,
