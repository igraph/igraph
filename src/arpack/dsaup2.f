...
c        %----------------------------------------------------%
c
         call igraphdcopy(kplusp, ritz, 1, workl(kplusp+1), 1)
         call igraphdcopy(kplusp, bounds, 1, workl(2*kplusp+1), 1)
c
c        %---------------------------------------------------%
...
c        %-------------------%
c
         call igraphdcopy (nev, bounds(np+1), 1, workl(np+1), 1)
...
c
               ...
               nevm2 = nev0 - nevd2 
               if ( nev .gt. 1 ) then
                  call igraphdswap ( min(nevd2,np), ritz(nevm2+1), 1,
     &                 ritz( max(kplusp-nevd2+1,kplusp-np+1) ), 1)
                  call igraphdswap ( min(nevd2,np), bounds(nevm2+1), 1,
...
c        %---------------------------------------------------------%
c
         if (ishift .eq. 0) call igraphdcopy (np, workl, 1, ritz, 1)
c
         if (msglvl .gt. 2) then
            call igraphivout (logfil, 1, np, ndigit,
...
c
         ...
         if (bmat .eq. 'G') then
            nbx = nbx + 1
            call igraphdcopy (n, resid, 1, workd(n+1), 1)
            ipntr(1) = n + 1
            ipntr(2) = 1
            ido = 2
c 
...
c 
            ...
            call igraphdcopy (n, resid, 1, workd, 1)
         end if
c 
  100    continue
c 
...
c 
         if (bmat .eq. 'G') then         
            rnorm = igraphddot (n, resid, 1, workd, 1)
