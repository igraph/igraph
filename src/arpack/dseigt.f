...
c          main diagonal in IGRAPHARPACKSECOND column.
c
c  LDH     Integer.  (INPUT)
...
c          Error exit flag from IGRAPHARPACKDSTQRB.
c
c\EndDoc
c
c-----------------------------------------------------------------------
...
c
c\Routines called:
c     IGRAPHARPACKDSTQRB  ARPACK routine that computes the eigenvalues and the
...
c     %----------------------%
c
      external   IGRAPHARPACKDCOPY, IGRAPHARPACKDSTQRB, IGRAPHARPACKDVOUT, IGRAPHARPACKSECOND
c
c     %-----------------------%
...
c
      if (msglvl .gt. 0) then
         call IGRAPHARPACKDVOUT (logfil, n, h(1,2), ndigit,
     &              '_seigt: main diagonal of matrix H')
         if (n .gt. 1) then
         call IGRAPHARPACKDVOUT (logfil, n-1, h(2,1), ndigit,
     &              '_seigt: sub diagonal of matrix H')
         end if
      end if
c
      call IGRAPHARPACKDCOPY  (n, h(1,2), 1, eig, 1)
      call IGRAPHARPACKDCOPY  (n-1, h(2,1), 1, workl, 1)
      call IGRAPHARPACKDSTQRB (n, eig, workl, bounds, workl(n+1), ierr)
