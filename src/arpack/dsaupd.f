...
c
c  IGRAPHARPACKDSAUPD is usually called iteratively to solve one of the 
...
c          call to IGRAPHARPACKDSAUPD.  IDO will be set internally to
c          indicate the type of operation to be performed.  Control is
c          then given back to the calling routine which has the
c          responsibility to carry out the requested operation and call
c          IGRAPHARPACKDSAUPD with the result.  The operand is given in
...
c          Must be 1,2,3,4,5; See under \Description of IGRAPHARPACKDSAUPD for the 
c          five modes available.
c
c          IPARAM(8) = NP
c          When ido = 3 and the user provides shifts through reverse
c          communication (IPARAM(1)=0), IGRAPHARPACKDSAUPD returns NP, the number
...
c
c          Note: IPNTR(8:10) is only referenced by IGRAPHARPACKDSEUPD. See Remark 2.
...
c          subroutine IGRAPHARPACKDSEUPD uses this output.
...
c                Informatinal error from LAPACK routine IGRAPHARPACKDSTEQR.
...
c     with the ARPACK subroutine IGRAPHARPACKDSEUPD. 
c
c  2. If the Ritz vectors corresponding to the converged Ritz values
c     are needed, the user must call IGRAPHARPACKDSEUPD immediately following completion
c     of IGRAPHARPACKDSAUPD. This is new starting with version 2.1 of ARPACK.
c
...
c
c\Routines called:
c     IGRAPHARPACKDSAUP2  ARPACK routine that implements the Implicitly Restarted
c             Arnoldi Iteration.
c     IGRAPHARPACKDSTATS  ARPACK routine that initialize timing and other statistics
c             variables.
...
c     %----------------------%
c
      external   IGRAPHARPACKDSAUP2,  IGRAPHARPACKDVOUT, IGRAPHARPACKIVOUT, IGRAPHARPACKSECOND, IGRAPHARPACKDSTATS
c
c     %--------------------%
...
c     | error within IGRAPHARPACKDSAUP2.               |
c     %------------------------------------%
c
...
c
     ...
     &               '_saupd: number of "converged" Ritz values')
         call IGRAPHARPACKDVOUT (logfil, np, workl(Ritz), ndigit, 
