...
c
c  where OP and B are as in IGRAPHARPACKDNAUPD.  The B-norm of r_{k+p} is also
c  computed and returned.
c
c\Usage:
c  call IGRAPHARPACKDNAITR
...
c          semi-inner product for the operator OP.  See IGRAPHARPACKDNAUPD.
...
c
c\Routines called:
c     IGRAPHARPACKDGETV0  ARPACK routine to generate the initial vector.
...
c        p_{j} = p_{j}/betaj
c     3) r_{j} = OP*v_{j} where OP is defined as in IGRAPHARPACKDNAUPD
...
c     %----------------------%
c
      external   IGRAPHARPACKDAXPY, IGRAPHARPACKDCOPY, IGRAPHARPACKDSCAL, IGRAPHARPACKDGEMV, IGRAPHARPACKDGETV0, IGRAPHARPACKDLABAD, 
     &           IGRAPHARPACKDVOUT, IGRAPHARPACKDMOUT, IGRAPHARPACKIVOUT, IGRAPHARPACKSECOND
c
c     %--------------------%
...
c
      Double precision
     &           IGRAPHARPACKDDOT, IGRAPHARPACKDNRM2, IGRAPHARPACKDLANHS, IGRAPHARPACKDLAMCH
      external   IGRAPHARPACKDDOT, IGRAPHARPACKDNRM2, IGRAPHARPACKDLANHS, IGRAPHARPACKDLAMCH
c
c     %---------------------%
...
c
         if (msglvl .gt. 1) then
            call IGRAPHARPACKIVOUT (logfil, 1, j, ndigit, 
...
c           %--------------------------------------%
c
            call IGRAPHARPACKDGETV0 (ido, bmat, itry, .false., n, j, v, ldv, 
...
c        %---------------------------------------------------------%
c
         call IGRAPHARPACKDCOPY (n, resid, 1, v(1,j), 1)
         if (rnorm .ge. unfl) then
             temp1 = one / rnorm
             call IGRAPHARPACKDSCAL (n, temp1, v(1,j), 1)
             call IGRAPHARPACKDSCAL (n, temp1, workd(ipj), 1)
         else
c
c            %-----------------------------------------%
...
c            %-----------------------------------------%
c
             call IGRAPHARPACKDLASCL ('General', i, i, rnorm, one, n, 1, 
     &                    v(1,j), n, infol)
             call IGRAPHARPACKDLASCL ('General', i, i, rnorm, one, n, 1, 
     &                    workd(ipj), n, infol)
         end if
...
c
         ...
         call IGRAPHARPACKDCOPY (n, v(1,j), 1, workd(ivj), 1)
         ipntr(1) = ivj
         ipntr(2) = irj
         ipntr(3) = ipj
         ido = 1
...
c        %------------------------------------------%
c
         call IGRAPHARPACKDCOPY (n, workd(irj), 1, resid, 1)
c 
c        %---------------------------------------%
...
c 
            ...
            call IGRAPHARPACKDCOPY (n, resid, 1, workd(ipj), 1)
         end if
   60    continue
c 
c        %----------------------------------%
...
c
         if (bmat .eq. 'G') then  
             wnorm = IGRAPHARPACKDDOT (n, resid, 1, workd(ipj), 1)
             wnorm = sqrt(abs(wnorm))
         else if (bmat .eq. 'I') then
            wnorm = IGRAPHARPACKDNRM2(n, resid, 1)
         end if
c
c        %-----------------------------------------%
...
c        %------------------------------------------%
c 
         call IGRAPHARPACKDGEMV ('T', n, j, one, v, ldv, workd(ipj), 1,
     &               zero, h(1,j), 1)
c
c        %--------------------------------------%
...
c        %--------------------------------------%
c
         call IGRAPHARPACKDGEMV ('N', n, j, -one, v, ldv, h(1,j), 1,
     &               one, resid, 1)
c
         if (j .gt. 1) h(j,j-1) = betaj
c
         call IGRAPHARPACKSECOND (t4)
c 
         orth1 = .true.
c
         call IGRAPHARPACKSECOND (t2)
         if (bmat .eq. 'G') then
            nbx = nbx + 1
            call IGRAPHARPACKDCOPY (n, resid, 1, workd(irj), 1)
            ipntr(1) = irj
            ipntr(2) = ipj
            ido = 2
c 
...
c
         if (bmat .eq. 'G') then         
            rnorm = IGRAPHARPACKDDOT (n, resid, 1, workd(ipj), 1)
...
c
         if (msglvl .gt. 2) then
            xtemp(1) = wnorm
            xtemp(2) = rnorm
            call IGRAPHARPACKDVOUT (logfil, 2, xtemp, ndigit, 
     &           '_naitr: re-orthonalization; wnorm and rnorm are')
            call IGRAPHARPACKDVOUT (logfil, j, h(1,j), ndigit,
...
c        %---------------------------------------------%
c
         call IGRAPHARPACKDGEMV ('N', n, j, -one, v, ldv, workd(irj), 1, 
     &               one, resid, 1)
         call IGRAPHARPACKDAXPY (j, one, workd(irj), 1, h(1,j), 1)
c 
         orth2 = .true.
         call IGRAPHARPACKSECOND (t2)
         if (bmat .eq. 'G') then
...
c 
         if (bmat .eq. 'G') then         
             rnorm1 = IGRAPHARPACKDDOT (n, resid, 1, workd(ipj), 1)
             rnorm1 = sqrt(abs(rnorm1))
         else if (bmat .eq. 'I') then
             rnorm1 = IGRAPHARPACKDNRM2(n, resid, 1)
         end if
c
...
c     
               ...
               if( tst1.eq.zero )
     &              tst1 = IGRAPHARPACKDLANHS( '1', k+np, h, ldh, workd(n+1) )
               if( abs( h( i+1,i ) ).le.max( ulp*tst1, smlnum ) ) 
     &              h(i+1,i) = zero
 110        continue
c     
            if (msglvl .gt. 2) then
               call IGRAPHARPACKDMOUT (logfil, k+np, k+np, h, ldh, ndigit, 
