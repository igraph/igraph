...
c               the ARPACK code IGRAPHARPACKDSORTR
c
c\SCCS Information: @(#) 
...
c-----------------------------------------------------------------------
c
      subroutine IGRAPHARPACKDSESRT (which, apply, n, x, na, a, lda)
c
c     %------------------%
c     | Scalar Arguments |
c     %------------------%
...
c
            if (x(j).lt.x(j+igap)) then
               ...
               x(j) = x(j+igap)
               x(j+igap) = temp
               if (apply) call IGRAPHARPACKDSWAP( na, a(1, j), 1, a(1,j+igap), 1)
            else
               go to 30
            endif
            j = j-igap
