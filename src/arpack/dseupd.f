...
c     %----------------------%
c
      external   igraphdcopy  , igraphdger   , igraphdgeqr2 , 
     &           igraphdlacpy , igraphdorm2r , igraphdscal , 
     &           igraphdsesrt , igraphdsteqr , igraphdswap  , 
     &           igraphdvout  , igraphivout , igraphdsortr 
c
c     %--------------------%
...
c
      Double precision 
     &           igraphdnrm2 , igraphdlamch 
      external   igraphdnrm2 , igraphdlamch 
c
c     %---------------------%
...
c     %---------------------------------%
c
      eps23 = igraphdlamch ('Epsilon-Machine') 
...
c        %-----------------------------------------------------------%
c
         call igraphdcopy (ncv-1, workl(ih+1), 1, workl(ihb), 1)
         call igraphdcopy (ncv, workl(ih+ldh), 1, workl(ihd), 1)
c
         call igraphdsteqr ('Identity', ncv, workl(ihd), workl(ihb),
...
c
         if (msglvl .gt. 1) then
            call igraphdcopy (ncv, workl(iq+ncv-1), ldq, workl(iw), 1)
...
c
               ...
               call igraphdcopy (ncv, workl(iq+ncv*(leftptr-1)), 1,
     &                    workl(iw), 1)
               call igraphdcopy (ncv, workl(iq+ncv*(rghtptr-1)), 1,
     &                    workl(iq+ncv*(leftptr-1)), 1)
               call igraphdcopy (ncv, workl(iw), 1,
...
c        %----------------------------------------%
c
         call igraphdcopy (nconv, workl(ihd), 1, d, 1)
c
      else
c
c        %-----------------------------------------------------%
...
c        %-----------------------------------------------------%
c
         call igraphdcopy (nconv, workl(ritz), 1, d, 1)
         call igraphdcopy (ncv, workl(ritz), 1, workl(ihd), 1)
c
      end if
c
c     %------------------------------------------------------------------%
...
c
     ...
     &                         ldq)
            ...
            call igraphdcopy (ncv, workl(bounds), 1, workl(ihb), 1)
         end if
c
      else 
c 
...
c        %-------------------------------------------------------------%
c
         call igraphdcopy  (ncv, workl(ihd), 1, workl(iw), 1)
...
c
     ...
     &                         ldq)
            ...
            call igraphdcopy (ncv, workl(bounds), 1, workl(ihb), 1)
            call igraphdscal (ncv, bnorm2/rnorm, workl(ihb), 1)
...
c        %----------------------------------------------------------%
c     
         call igraphdgeqr2 (ncv, nconv        , workl(iq) ,
...
c        %--------------------------------------------------------%
c     
         call igraphdorm2r ('Right', 'Notranspose', n        ,
     &                ncv    , nconv        , workl(iq),
     &                ldq    , workl(iw+ncv), v        ,
     &                ldv    , workd(n+1)   , ierr)
         call igraphdlacpy ('All', n, nconv, v, ldv, z, ldz)
c
c        %-----------------------------------------------------%
...
c
  ...
  65     continue
         workl(ihb+ncv-1) = one
         call igraphdorm2r ('Left', 'Transpose'  , ncv       ,
     &                1     , nconv        , workl(iq) ,
     &                ldq   , workl(iw+ncv), workl(ihb),
     &                ncv   , temp         , ierr)
c
...
c        %-------------------------------------------------%
c
         call igraphdscal  (ncv, bnorm2, workl(ihb), 1)
         if (type .eq. 'SHIFTI') then 
c
            do 80 k=1, ncv
...
c
      if (type .ne. 'REGULR')
     &   call igraphdger  (n, nconv, one, resid, 1, workl(iw), 1, z, 
     &                     ldz)
c
 9000 continue
c
