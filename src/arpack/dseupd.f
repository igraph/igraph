...
c  by IGRAPHARPACKDSAUPD for the linear operator OP prescribed by the MODE selection
c  (see IPARAM(7) in IGRAPHARPACKDSAUPD documentation.)  IGRAPHARPACKDSAUPD must be called before
...
c
c  See documentation in the header of the subroutine IGRAPHARPACKDSAUPD for a definition 
...
c          the Ritz values of OP computed by IGRAPHARPACKDSAUPD transformed to
...
c          Arnoldi/Lanczos basis array V computed by IGRAPHARPACKDSAUPD.
c
c  LDZ     Integer.  (INPUT)
...
c
c         must be passed directly to IGRAPHARPACKDSEUPD following the last call
c         to IGRAPHARPACKDSAUPD.  These arguments MUST NOT BE MODIFIED between
c         the the last call to IGRAPHARPACKDSAUPD and the call to IGRAPHARPACKDSEUPD.
c
...
c          IGRAPHARPACKDSAUPD.  They are not changed by IGRAPHARPACKDSEUPD.
...
c          of the above information computed by IGRAPHARPACKDSEUPD.
c          -------------------------------------------------------------
...
c                     IGRAPHARPACKDSEUPD if RVEC = .TRUE. See Remarks.
c          -------------------------------------------------------------
c
...
c                Information error from LAPACK routine IGRAPHARPACKDSTEQR.
c          = -9: Starting vector is zero.
c          = -10: IPARAM(7) must be 1,2,3,4,5.
c          = -11: IPARAM(7) = 1 and BMAT = 'G' are incompatible.
c          = -12: NEV and WHICH = 'BE' are incompatible.
c          = -14: IGRAPHARPACKDSAUPD did not find any eigenvalues to sufficient
c                 accuracy.
...
c
c\Routines called:
c     IGRAPHARPACKDSESRT  ARPACK routine that sorts an array X, and applies the
c             corresponding permutation to a matrix A.
c     IGRAPHARPACKDSORTR  IGRAPHARPACKDSORTR  ARPACK sorting routine.
...
c
c-----------------------------------------------------------------------
      subroutine IGRAPHARPACKDSEUPD (rvec, howmny, select, d, z, ldz, sigma, bmat,
...
c     %----------------------%
c
      external   IGRAPHARPACKDCOPY, IGRAPHARPACKDGER, IGRAPHARPACKDGEQR2, IGRAPHARPACKDLACPY, IGRAPHARPACKDORM2R, IGRAPHARPACKDSCAL, 
     &           IGRAPHARPACKDSESRT, IGRAPHARPACKDSTEQR, IGRAPHARPACKDSWAP, IGRAPHARPACKDVOUT, IGRAPHARPACKIVOUT, IGRAPHARPACKDSORTR
c
c     %--------------------%
...
c
      Double precision
     &           IGRAPHARPACKDNRM2, IGRAPHARPACKDLAMCH
      external   IGRAPHARPACKDNRM2, IGRAPHARPACKDLAMCH
c
c     %---------------------%
...
c     |       IGRAPHARPACKDSAUPD stores the B-norm of the last residual   |
...
c     |       The wanted values are in the first NCONV spots. |
c     | NOTE: workl(1:4*ncv) is set by IGRAPHARPACKDSAUPD and is not      |
c     |       modified by IGRAPHARPACKDSEUPD.                             |
c     %-------------------------------------------------------%
c
c     %-------------------------------------------------------%
c     | The following is used and set by IGRAPHARPACKDSEUPD.              |
...
c     |       IGRAPHARPACKDSTEQR. Not referenced if RVEC = .False.        |
c     |       Ordering follows that of workl(4*ncv+1:5*ncv)   |
c     | workl(6*ncv+ncv*ncv+1:6*ncv+ncv*ncv+2*ncv) :=         |
c     |       Workspace. Needed by IGRAPHARPACKDSTEQR and by IGRAPHARPACKDSEUPD.      |
...
c     | Upon exit of IGRAPHARPACKDSAUPD WORKD(1:N) has    |
c     | B*RESID(1:N).                         |
c     %---------------------------------------%
c
      rnorm = workl(ih)
      if (bmat .eq. 'I') then
         ...
         bnorm2 = IGRAPHARPACKDNRM2(n, workd, 1)
      end if
c
      if (rvec) then
c
...
c
             if (msglvl .gt. 2) then
                call IGRAPHARPACKDVOUT(logfil, 1, thres1, ndigit,
...
c
             if (msglvl .gt. 2) then
                kv(1) = thres1
                kv(2) = thres2
                call IGRAPHARPACKDVOUT(logfil, 2, kv, ndigit,
...
c        %-----------------------------------------------------------%
c
         call IGRAPHARPACKDCOPY (ncv-1, workl(ih+1), 1, workl(ihb), 1)
         call IGRAPHARPACKDCOPY (ncv, workl(ih+ldh), 1, workl(ihd), 1)
c
         call IGRAPHARPACKDSTEQR ('Identity', ncv, workl(ihd), workl(ihb),
...
c
         if (msglvl .gt. 1) then
            call IGRAPHARPACKDCOPY (ncv, workl(iq+ncv-1), ldq, workl(iw), 1)
            call IGRAPHARPACKDVOUT (logfil, ncv, workl(ihd), ndigit,
     &          '_seupd: NCV Ritz values of the final H matrix')
            call IGRAPHARPACKDVOUT (logfil, ncv, workl(iw), ndigit,
...
c
               ...
               call IGRAPHARPACKDCOPY(ncv, workl(iq+ncv*(leftptr-1)), 1,
     &                    workl(iw), 1)
               call IGRAPHARPACKDCOPY(ncv, workl(iq+ncv*(rghtptr-1)), 1,
     &                    workl(iq+ncv*(leftptr-1)), 1)
               call IGRAPHARPACKDCOPY(ncv, workl(iw), 1,
...
c        %----------------------------------------%
c
         call IGRAPHARPACKDCOPY(nconv, workl(ihd), 1, d, 1)
c
      else
c
c        %-----------------------------------------------------%
...
c        %-----------------------------------------------------%
c
         call IGRAPHARPACKDCOPY (nconv, workl(ritz), 1, d, 1)
         call IGRAPHARPACKDCOPY (ncv, workl(ritz), 1, workl(ihd), 1)
c
      end if
c
c     %------------------------------------------------------------------%
...
c
         if (rvec) then
            call IGRAPHARPACKDSESRT ('LA', rvec , nconv, d, ncv, workl(iq), ldq)
         else
            call IGRAPHARPACKDCOPY (ncv, workl(bounds), 1, workl(ihb), 1)
         end if
c
      else 
c 
...
c        |    where the theta are the Ritz values returned by IGRAPHARPACKDSAUPD.  |
c        | NOTES:                                                      |
c        | *The Ritz vectors are not affected by the transformation.   |
c        |  They are only reordered.                                   |
c        %-------------------------------------------------------------%
c
         call IGRAPHARPACKDCOPY (ncv, workl(ihd), 1, workl(iw), 1)
...
c
         call IGRAPHARPACKDCOPY (nconv, workl(ihd), 1, d, 1)
         call IGRAPHARPACKDSORTR ('LA', .true., nconv, workl(ihd), workl(iw))
         if (rvec) then
            call IGRAPHARPACKDSESRT ('LA', rvec , nconv, d, ncv, workl(iq), ldq)
         else
            call IGRAPHARPACKDCOPY (ncv, workl(bounds), 1, workl(ihb), 1)
            call IGRAPHARPACKDSCAL (ncv, bnorm2/rnorm, workl(ihb), 1)
            call IGRAPHARPACKDSORTR ('LA', .true., nconv, d, workl(ihb))
         end if
c
      end if 
c 
...
c        %----------------------------------------------------------%
c     
         call IGRAPHARPACKDGEQR2 (ncv, nconv, workl(iq), ldq, workl(iw+ncv), 
     &        workl(ihb), ierr)
c
c     
c        %--------------------------------------------------------%
...
c        %--------------------------------------------------------%
c     
         call IGRAPHARPACKDORM2R ('Right', 'Notranspose', n, ncv, nconv, workl(iq),
...
c
  ...
  65     continue
         workl(ihb+ncv-1) = one
         call IGRAPHARPACKDORM2R ('Left', 'Transpose', ncv, 1, nconv, workl(iq),
...
c        |              as computed by IGRAPHARPACKDSAUPD.             |
c        | *  Determine Ritz estimates of the lambda.      |
c        %-------------------------------------------------%
c
         call IGRAPHARPACKDSCAL (ncv, bnorm2, workl(ihb), 1)
         if (type .eq. 'SHIFTI') then 
c
            do 80 k=1, ncv
...
c
      if (type .ne. 'REGULR' .and. msglvl .gt. 1) then
         call IGRAPHARPACKDVOUT (logfil, nconv, d, ndigit,
     &          '_seupd: Untransformed converged Ritz values')
         call IGRAPHARPACKDVOUT (logfil, nconv, workl(ihb), ndigit, 
...
c
      if (type .ne. 'REGULR')
     &   call IGRAPHARPACKDGER (n, nconv, one, resid, 1, workl(iw), 1, z, ldz)
c
 9000 continue
c
      return
