...
c
c  IGRAPHARPACKDNAUPD is usually called iteratively to solve one of the 
...
c          call to IGRAPHARPACKDNAUPD.  IDO will be set internally to
c          indicate the type of operation to be performed.  Control is
c          then given back to the calling routine which has the
c          responsibility to carry out the requested operation and call
c          IGRAPHARPACKDNAUPD with the result.  The operand is given in
...
c          DEFAULT = IGRAPHARPACKDLAMCH('EPS')  (machine precision as computed
c                    by the LAPACK auxiliary subroutine IGRAPHARPACKDLAMCH).
c
...
c          Must be 1,2,3,4; See under \Description of IGRAPHARPACKDNAUPD for the 
c          four modes available.
c
c          IPARAM(8) = NP
c          When ido = 3 and the user provides shifts through reverse
c          communication (IPARAM(1)=0), IGRAPHARPACKDNAUPD returns NP, the number
...
c
c          Note: IPNTR(9:13) is only referenced by IGRAPHARPACKDNEUPD. See Remark 2 below.
c
...
c          2 below, subroutine IGRAPHARPACKDNEUPD uses this output.
...
c     original problem may be obtained with the ARPACK subroutine IGRAPHARPACKDNEUPD.
c
c  2. If a basis for the invariant subspace corresponding to the converged Ritz 
c     values is needed, the user must call IGRAPHARPACKDNEUPD immediately following 
c     completion of IGRAPHARPACKDNAUPD. This is new starting with release 2 of ARPACK.
c
...
c
c\Routines called:
c     IGRAPHARPACKDNAUP2  ARPACK routine that implements the Implicitly Restarted
c             Arnoldi Iteration.
...
c     %----------------------%
c
      external   IGRAPHARPACKDNAUP2, IGRAPHARPACKDVOUT, IGRAPHARPACKIVOUT, IGRAPHARPACKSECOND, IGRAPHARPACKDSTATN
c
c     %--------------------%
...
c
         if (nb .le. 0)				nb = 1
         if (tol .le. zero)			tol = IGRAPHARPACKDLAMCH('EpsMach')
c
c        %----------------------------------------------%
...
c        | The final workspace is needed by subroutine IGRAPHARPACKDNEIGH called   |
c        | by IGRAPHARPACKDNAUP2. Subroutine IGRAPHARPACKDNEIGH calls LAPACK routines for      |
...
c     | error within IGRAPHARPACKDNAUP2.               |
c     %------------------------------------%
c
...
c
      if (msglvl .gt. 0) then
         call IGRAPHARPACKIVOUT (logfil, 1, mxiter, ndigit,
     &               '_naupd: Number of update iterations taken')
         call IGRAPHARPACKIVOUT (logfil, 1, np, ndigit,
     &               '_naupd: Number of wanted "converged" Ritz values')
         call IGRAPHARPACKDVOUT (logfil, np, workl(ritzr), ndigit, 
     &               '_naupd: Real part of the final Ritz values')
         call IGRAPHARPACKDVOUT (logfil, np, workl(ritzi), ndigit, 
     &               '_naupd: Imaginary part of the final Ritz values')
         call IGRAPHARPACKDVOUT (logfil, np, workl(bounds), ndigit, 
