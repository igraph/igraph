...
c
c  This is mostly a modification of the LAPACK routine IGRAPHARPACKDLAHQR.
c  
c\Usage:
c  call IGRAPHARPACKDLAQRB
...
c     IGRAPHARPACKDLANV2  LAPACK routine that computes the Schur factorization of
...
c     IGRAPHARPACKDROT    Level 1 BLAS that applies a rotation to a 2 by 2 matrix.
...
c               Modified from the LAPACK routine IGRAPHARPACKDLAHQR so that only the
c               last component of the Schur vectors are computed.
c
c\SCCS Information: @(#) 
...
c-----------------------------------------------------------------------
c
      subroutine IGRAPHARPACKDLAQRB ( wantt, n, ilo, ihi, h, ldh, wr, wi,
     &                    z, info )
c
c     %------------------%
c     | Scalar Arguments |
...
c
      Double precision
     &           IGRAPHARPACKDLAMCH, IGRAPHARPACKDLANHS
      external   IGRAPHARPACKDLAMCH, IGRAPHARPACKDLANHS
c
c     %----------------------%
c     | External Subroutines |
c     %----------------------%
c
      external   IGRAPHARPACKDCOPY, IGRAPHARPACKDLABAD, IGRAPHARPACKDLANV2, IGRAPHARPACKDLARFG, IGRAPHARPACKDROT
c
c     %-----------------------%
...
c
         do 20 k = i, l + 1, -1
            ...
            if( tst1.eq.zero )
     &         tst1 = IGRAPHARPACKDLANHS( '1', i-l+1, h( l, l ), ldh, work )
...
c 
            nr = min( 3, i-k+1 )
            if( k.gt.m )
     &         call IGRAPHARPACKDCOPY( nr, h( k, k-1 ), 1, v, 1 )
            call IGRAPHARPACKDLARFG( nr, v( 1 ), v( 2 ), 1, t1 )
            if( k.gt.m ) then
               h( k, k-1 ) = v( 1 )
               h( k+1, k-1 ) = zero
               if( k.lt.i-1 )
...
c        %--------------------------------------------------------%
c
         call IGRAPHARPACKDLANV2( h( i-1, i-1 ), h( i-1, i ), h( i, i-1 ),
...
c
            if( i2.gt.i )
     &         call IGRAPHARPACKDROT( i2-i, h( i-1, i+1 ), ldh, h( i, i+1 ), ldh,
     &                    cs, sn )
            call IGRAPHARPACKDROT( i-i1-1, h( i1, i-1 ), 1, h( i1, i ), 1, cs, sn )
            sum      = cs*z( i-1 ) + sn*z( i )
            z( i )   = cs*z( i )   - sn*z( i-1 )
            z( i-1 ) = sum
         end if
