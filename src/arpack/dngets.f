...
c-----------------------------------------------------------------------
c
      subroutine IGRAPHARPACKDNGETS ( ishift, which, kev, np, ritzr, ritzi, bounds,
     &                    shiftr, shifti )
c
c     %----------------------------------------------------%
...
c     %----------------------%
c
      external   IGRAPHARPACKDCOPY, IGRAPHARPACKDSORTC, IGRAPHARPACKSECOND
c
c     %----------------------%
...
c
      if (which .eq. 'LM') then
         call IGRAPHARPACKDSORTC ('LR', .true., kev+np, ritzr, ritzi, bounds)
      else if (which .eq. 'SM') then
         call IGRAPHARPACKDSORTC ('SR', .true., kev+np, ritzr, ritzi, bounds)
      else if (which .eq. 'LR') then
         call IGRAPHARPACKDSORTC ('LM', .true., kev+np, ritzr, ritzi, bounds)
      else if (which .eq. 'SR') then
         call IGRAPHARPACKDSORTC ('SM', .true., kev+np, ritzr, ritzi, bounds)
...
c      
      call IGRAPHARPACKDSORTC (which, .true., kev+np, ritzr, ritzi, bounds)
c     
c     %-------------------------------------------------------%
...
c        | are applied in subroutine IGRAPHARPACKDNAPPS.                     |
c        | Be careful and use 'SR' since we want to sort BOUNDS! |
c        %-------------------------------------------------------%
c     
         call IGRAPHARPACKDSORTC ( 'SR', .true., np, bounds, ritzr, ritzi )
      end if
c     
...
c
      if (msglvl .gt. 0) then
         call IGRAPHARPACKIVOUT (logfil, 1, kev, ndigit, '_ngets: KEV is')
         call IGRAPHARPACKIVOUT (logfil, 1, np, ndigit, '_ngets: NP is')
         call IGRAPHARPACKDVOUT (logfil, kev+np, ritzr, ndigit,
     &        '_ngets: Eigenvalues of current H matrix -- real part')
         call IGRAPHARPACKDVOUT (logfil, kev+np, ritzi, ndigit,
