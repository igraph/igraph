...
c          Upon, entry to IGRAPHARPACKDNAPPS, the shifts must be sorted so that the 
...
c     IGRAPHARPACKDLABAD  LAPACK routine that computes machine constants.
c     IGRAPHARPACKDLACPY  LAPACK matrix copy routine.
c     IGRAPHARPACKDLAMCH  LAPACK routine that determines machine constants. 
c     IGRAPHARPACKDLANHS  LAPACK routine that computes various norms of a matrix.
c     IGRAPHARPACKDLAPY2  LAPACK routine to compute sqrt(x**2+y**2) carefully.
c     IGRAPHARPACKDLARF   LAPACK routine that applies Householder reflection to
c             a matrix.
c     IGRAPHARPACKDLARFG  LAPACK Householder reflection construction routine.
...
c     IGRAPHARPACKDGEMV   Level 2 BLAS routine for matrix vector multiplication.
...
c     comes from. Deflation as in LAPACK routine IGRAPHARPACKDLAHQR (QR algorithm
...
c     %----------------------%
c
      external   IGRAPHARPACKDAXPY, IGRAPHARPACKDCOPY, IGRAPHARPACKDSCAL, IGRAPHARPACKDLACPY, IGRAPHARPACKDLARFG, IGRAPHARPACKDLARF,
     &           IGRAPHARPACKDLASET, IGRAPHARPACKDLABAD, IGRAPHARPACKSECOND, IGRAPHARPACKDLARTG
c
c     %--------------------%
...
c
      Double precision
     &           IGRAPHARPACKDLAMCH, IGRAPHARPACKDLANHS, IGRAPHARPACKDLAPY2
      external   IGRAPHARPACKDLAMCH, IGRAPHARPACKDLANHS, IGRAPHARPACKDLAPY2
c
c     %----------------------%
...
c        | REFERENCE: LAPACK subroutine IGRAPHARPACKDLAHQR           |
c        %-----------------------------------------------%
c
...
c     %--------------------------------------------%
c
      call IGRAPHARPACKDLASET ('All', kplusp, kplusp, zero, one, q, ldq)
c
c     %----------------------------------------------%
...
c
         if (msglvl .gt. 2 ) then
            call IGRAPHARPACKIVOUT (logfil, 1, jj, ndigit, 
     &               '_napps: shift number.')
            call IGRAPHARPACKDVOUT (logfil, 1, sigmar, ndigit, 
     &               '_napps: The real part of the shift ')
            call IGRAPHARPACKDVOUT (logfil, 1, sigmai, ndigit, 
...
c
            ...
            if( tst1.eq.zero )
     &         tst1 = IGRAPHARPACKDLANHS( '1', kplusp-jj+1, h, ldh, workl )
            if( abs( h( i+1,i ) ).le.max( ulp*tst1, smlnum ) ) then
               if (msglvl .gt. 0) then
                  call IGRAPHARPACKIVOUT (logfil, 1, i, ndigit, 
     &                 '_napps: matrix splitting at row/column no.')
                  call IGRAPHARPACKIVOUT (logfil, 1, jj, ndigit, 
     &                 '_napps: matrix splitting with shift number.')
                  call IGRAPHARPACKDVOUT (logfil, 1, h(i+1,i), ndigit, 
...
c
         if (msglvl .gt. 2) then
             call IGRAPHARPACKIVOUT (logfil, 1, istart, ndigit, 
     &                   '_napps: Start of current block ')
             call IGRAPHARPACKIVOUT (logfil, 1, iend, ndigit, 
...
c              %-----------------------------------------------------%
c
               call IGRAPHARPACKDLARTG (f, g, c, s, r)
               if (i .gt. istart) then
c
c                 %-------------------------------------------%
...
c
            s    = 2.0*sigmar
            t = IGRAPHARPACKDLAPY2 ( sigmar, sigmai ) 
...
c              %-----------------------------------------------------%
c
               call IGRAPHARPACKDLARFG ( nr, u(1), u(2), 1, tau )
c
               if (i .gt. istart) then
                  h(i,i-1)   = u(1)
                  h(i+1,i-1) = zero
...
c              %--------------------------------------%
c
               call IGRAPHARPACKDLARF ('Left', nr, kplusp-i+1, u, 1, tau,
     &                     h(i,i), ldh, workl)
c
c              %---------------------------------------%
...
c
               ir = min ( i+3, iend )
               call IGRAPHARPACKDLARF ('Right', ir, nr, u, 1, tau,
     &                     h(1,i), ldh, workl)
c
c              %-----------------------------------------------------%
...
c              %-----------------------------------------------------%
c
               call IGRAPHARPACKDLARF ('Right', kplusp, nr, u, 1, tau, 
     &                     q(1,i), ldq, workl)
c
c              %----------------------------%
...
c
      do 120 j=1,kev
         if ( h(j+1,j) .lt. zero ) then
              call IGRAPHARPACKDSCAL( kplusp-j+1, -one, h(j+1,j), ldh )
              call IGRAPHARPACKDSCAL( min(j+2, kplusp), -one, h(1,j+1), 1 )
              call IGRAPHARPACKDSCAL( min(j+np+1,kplusp), -one, q(1,j+1), 1 )
         end if
 120  continue
c
      do 130 i = 1, kev
...
c
         tst1 = abs( h( i, i ) ) + abs( h( i+1, i+1 ) )
         if( tst1.eq.zero )
     &       tst1 = IGRAPHARPACKDLANHS( '1', kev, h, ldh, workl )
...
c
     ...
     &    call IGRAPHARPACKDGEMV ('N', n, kplusp, one, v, ldv, q(1,kev+1), 1, zero, 
     &                workd(n+1), 1)
c 
c     %----------------------------------------------------------%
...
c
      do 140 i = 1, kev
         call IGRAPHARPACKDGEMV ('N', n, kplusp-i+1, one, v, ldv,
     &               q(1,kev-i+1), 1, zero, workd, 1)
         call IGRAPHARPACKDCOPY (n, workd, 1, v(1,kplusp-i+1), 1)
  140 continue
c
c     %-------------------------------------------------%
...
c     %-------------------------------------------------%
c
      call IGRAPHARPACKDLACPY ('A', n, kev, v(1,kplusp-kev+1), ldv, v, ldv)
c 
c     %--------------------------------------------------------------%
...
c
      if (h(kev+1,kev) .gt. zero)
     &   call IGRAPHARPACKDCOPY (n, workd(n+1), 1, v(1,kev+1), 1)
c 
c     %-------------------------------------%
...
c     %-------------------------------------%
c
      call IGRAPHARPACKDSCAL (n, q(kplusp,kev), resid, 1)
      if (h(kev+1,kev) .gt. zero)
     &   call IGRAPHARPACKDAXPY (n, h(kev+1,kev), v(1,kev+1), 1, resid, 1)
c
      if (msglvl .gt. 1) then
         call IGRAPHARPACKDVOUT (logfil, 1, q(kplusp,kev), ndigit,
     &        '_napps: sigmak = (e_{kev+p}^T*Q)*e_{kev}')
         call IGRAPHARPACKDVOUT (logfil, 1, h(kev+1,kev), ndigit,
     &        '_napps: betak = e_{kev+1}^T*H*e_{kev}')
         call IGRAPHARPACKIVOUT (logfil, 1, kev, ndigit, 
     &               '_napps: Order of the final Hessenberg matrix ')
         if (msglvl .gt. 2) then
            call IGRAPHARPACKDMOUT (logfil, kev, kev, h, ldh, ndigit,
