...
c  call to IGRAPHARPACKDNAUPD.  IGRAPHARPACKDNAUPD must be called before this routine is called.
...
c  Schur basis.
c
c  See documentation in the header of the subroutine IGRAPHARPACKDNAUPD for 
...
c  IPARAM(7), MODE and WHICH in the documentation of IGRAPHARPACKDNAUPD.
c
c\Usage:
c  call IGRAPHARPACKDNEUPD 
...
c          IGRAPHARPACKDNAUPD. A further computation must be performed by the user
c          to transform the Ritz values computed for OP by IGRAPHARPACKDNAUPD to those
...
c          vector and the IGRAPHARPACKSECOND column holds the imaginary part.  The 
...
c          basis array V computed by IGRAPHARPACKDNAUPD.  In this case the Arnoldi basis
...
c  **** call to IGRAPHARPACKDNAUPD that was just completed.               ****
c
...
c
c         must be passed directly to IGRAPHARPACKDNEUPD following the last call
c         to IGRAPHARPACKDNAUPD.  These arguments MUST NOT BE MODIFIED between
c         the the last call to IGRAPHARPACKDNAUPD and the call to IGRAPHARPACKDNEUPD.
c
...
c                      vectors for OP as constructed by IGRAPHARPACKDNAUPD .
c
...
c          IGRAPHARPACKDNAUPD.  They are not changed by IGRAPHARPACKDNEUPD.
...
c          of the above information computed by IGRAPHARPACKDNEUPD.
c          -------------------------------------------------------------
...
c                     IGRAPHARPACKDNEUPD if RVEC = .TRUE. See Remark 2 below.
c          -------------------------------------------------------------
c
...
c          =  0: Normal exit.
c
c          =  1: The Schur form computed by LAPACK routine IGRAPHARPACKDLAHQR
c                could not be reordered by LAPACK routine IGRAPHARPACKDTRSEN.
c                Re-enter subroutine IGRAPHARPACKDNEUPD with IPARAM(5)=NCV and 
...
c                Informational error from LAPACK routine IGRAPHARPACKDLAHQR.
c          = -9: Error return from calculation of eigenvectors.
c                Informational error from LAPACK routine IGRAPHARPACKDTREVC.
c          = -10: IPARAM(7) must be 1,2,3,4.
c          = -11: IPARAM(7) = 1 and BMAT = 'G' are incompatible.
c          = -12: HOWMNY = 'S' not yet implemented
c          = -13: HOWMNY must be one of 'A' or 'P' if RVEC = .true.
c          = -14: IGRAPHARPACKDNAUPD did not find any eigenvalues to sufficient
c                 accuracy.
c
c\BeginLib
c
...
c
c\Routines called:
c     IGRAPHARPACKIVOUT   ARPACK utility routine that prints integers.
c     IGRAPHARPACKDMOUT   ARPACK utility routine that prints matrices
c     IGRAPHARPACKDVOUT   ARPACK utility routine that prints vectors.
c     IGRAPHARPACKDGEQR2  LAPACK routine that computes the QR factorization of 
c             a matrix.
c     IGRAPHARPACKDLACPY  LAPACK matrix copy routine.
c     IGRAPHARPACKDLAHQR  LAPACK routine to compute the real Schur form of an
...
c     IGRAPHARPACKDORM2R  LAPACK routine that applies an orthogonal matrix in 
c             factored form.
c     IGRAPHARPACKDTREVC  LAPACK routine to compute the eigenvectors of a matrix
c             in upper quasi-triangular form.
c     IGRAPHARPACKDTRSEN  LAPACK routine that re-orders the Schur form.
c     IGRAPHARPACKDTRMM   Level 3 BLAS matrix times an upper triangular matrix.
c     IGRAPHARPACKDGER    Level 2 BLAS rank one update to a matrix.
c     IGRAPHARPACKDCOPY   Level 1 BLAS that copies one vector to another .
c     IGRAPHARPACKDDOT    Level 1 BLAS that computes the scalar product of two vectors.
c     IGRAPHARPACKDNRM2   Level 1 BLAS that computes the norm of a vector.
c     IGRAPHARPACKDSCAL   Level 1 BLAS that scales a vector.
c
c\Remarks
c
...
c     values computed by IGRAPHARPACKDNAUPD for OP to those of A*z = lambda*B*z. 
...
c
c-----------------------------------------------------------------------
      subroutine IGRAPHARPACKDNEUPD (rvec, howmny, select, dr, di, z, ldz, sigmar, 
...
c     %----------------------%
c
      external   IGRAPHARPACKDCOPY, IGRAPHARPACKDGER, IGRAPHARPACKDGEQR2, IGRAPHARPACKDLACPY, IGRAPHARPACKDLAHQR, IGRAPHARPACKDLASET, IGRAPHARPACKDMOUT, 
     &           IGRAPHARPACKDORM2R, IGRAPHARPACKDTREVC, IGRAPHARPACKDTRMM, IGRAPHARPACKDTRSEN, IGRAPHARPACKDSCAL, IGRAPHARPACKDVOUT, IGRAPHARPACKIVOUT
c
c     %--------------------%
...
c
      Double precision
     &           IGRAPHARPACKDLAPY2, IGRAPHARPACKDNRM2, IGRAPHARPACKDLAMCH, IGRAPHARPACKDDOT
      external   IGRAPHARPACKDLAPY2, IGRAPHARPACKDNRM2, IGRAPHARPACKDLAMCH, IGRAPHARPACKDDOT
c
c     %---------------------%
...
c     %---------------------------------%
c
      eps23 = IGRAPHARPACKDLAMCH('Epsilon-Machine')
...
c
c     %-----------------------------------------------------------%
c     | The following is used and set by IGRAPHARPACKDNEUPD.                  |
...
c
         if (which .eq. 'LM' .or. which .eq. 'SM') then
            thres = IGRAPHARPACKDLAPY2( workl(ritzr), workl(ritzi) )
...
c
         if (msglvl .gt. 2) then
            call IGRAPHARPACKDVOUT(logfil, 1, thres, ndigit,
...
c
         ...
         do 10 j = 0, ncv-1
            select(j+1) = .false.
            if (which .eq. 'LM') then
               if (IGRAPHARPACKDLAPY2(workl(irr+j), workl(iri+j))
     &            .ge. thres) then
                  temp1 = max( eps23, 
     &                         IGRAPHARPACKDLAPY2( workl(irr+j), workl(iri+j) ) )
...
c
         if (msglvl .gt. 2) then
             call IGRAPHARPACKIVOUT(logfil, 1, ktrord, ndigit,
     &            '_neupd: Number of specified eigenvalues')
             call IGRAPHARPACKIVOUT(logfil, 1, nconv, ndigit,
     &            '_neupd: Number of "converged" eigenvalues')
         end if
c
c        %-----------------------------------------------------------%
c        | Call LAPACK routine IGRAPHARPACKDLAHQR to compute the real Schur form |
c        | of the upper Hessenberg matrix returned by IGRAPHARPACKDNAUPD.        |
c        | Make a copy of the upper Hessenberg matrix.               |
c        | Initialize the Schur vector matrix Q to the identity.     |
c        %-----------------------------------------------------------%
c     
         call IGRAPHARPACKDCOPY (ldh*ncv, workl(ih), 1, workl(iuptri), 1)
         call IGRAPHARPACKDLASET ('All', ncv, ncv, zero, one, workl(invsub), ldq)
         call IGRAPHARPACKDLAHQR (.true., .true., ncv, 1, ncv, workl(iuptri), ldh,
     &        workl(iheigr), workl(iheigi), 1, ncv, 
     &        workl(invsub), ldq, ierr)
         call IGRAPHARPACKDCOPY (ncv, workl(invsub+ncv-1), ldq, workl(ihbds), 1)
c     
         if (ierr .ne. 0) then
            info = -8
            go to 9000
...
c     
         if (msglvl .gt. 1) then
            call IGRAPHARPACKDVOUT (logfil, ncv, workl(iheigr), ndigit,
     &           '_neupd: Real part of the eigenvalues of H')
            call IGRAPHARPACKDVOUT (logfil, ncv, workl(iheigi), ndigit,
     &           '_neupd: Imaginary part of the Eigenvalues of H')
            call IGRAPHARPACKDVOUT (logfil, ncv, workl(ihbds), ndigit,
     &           '_neupd: Last row of the Schur vector matrix')
            if (msglvl .gt. 3) then
               call IGRAPHARPACKDMOUT (logfil, ncv, ncv, workl(iuptri), ldh, ndigit,
...
c           %-----------------------------------------------------%
c     
            call IGRAPHARPACKDTRSEN ('None', 'V', select, ncv, workl(iuptri), ldh, 
...
c
     ...
     &           '_neupd: Imag part of the eigenvalues of H--reordered')
                if (msglvl .gt. 3) then
                   call IGRAPHARPACKDMOUT (logfil, ncv, ncv, workl(iuptri), ldq, 
     &                  ndigit,
...
c
         if (type .eq. 'REGULR') then 
            call IGRAPHARPACKDCOPY (nconv, workl(iheigr), 1, dr, 1)
            call IGRAPHARPACKDCOPY (nconv, workl(iheigi), 1, di, 1)
         end if
c     
c        %----------------------------------------------------------%
...
c        %----------------------------------------------------------%
c     
         call IGRAPHARPACKDGEQR2 (ncv, nconv, workl(invsub), ldq, workev, 
     &        workev(ncv+1), ierr)
c
c        %---------------------------------------------------------%
c        | * Postmultiply V by Q using IGRAPHARPACKDORM2R.                     |   
...
c        %---------------------------------------------------------%
c     
         call IGRAPHARPACKDORM2R ('Right', 'Notranspose', n, ncv, nconv,
     &        workl(invsub), ldq, workev, v, ldv, workd(n+1), ierr)
         call IGRAPHARPACKDLACPY ('All', n, nconv, v, ldv, z, ldz)
c
         do 20 j=1, nconv
c     
c           %---------------------------------------------------%
...
c     
            if (workl(invsub+(j-1)*ldq+j-1) .lt. zero) then
               call IGRAPHARPACKDSCAL (nconv, -one, workl(iuptri+j-1), ldq)
               call IGRAPHARPACKDSCAL (nconv, -one, workl(iuptri+(j-1)*ldq), 1)
            end if
c     
 20      continue
c     
...
c     
 ...
 30         continue
c
            call IGRAPHARPACKDTREVC ('Right', 'Select', select, ncv, workl(iuptri), 
...
c           | IGRAPHARPACKDTREVC returns each eigenvector normalized so  |
...
c                 %----------------------%
c     
                  temp = IGRAPHARPACKDNRM2( ncv, workl(invsub+(j-1)*ldq), 1 )
                  call IGRAPHARPACKDSCAL ( ncv, one / temp, 
...
c
                  if (iconj .eq. 0) then
                     temp = IGRAPHARPACKDLAPY2( IGRAPHARPACKDNRM2( ncv, workl(invsub+(j-1)*ldq), 
     &                      1 ), IGRAPHARPACKDNRM2( ncv, workl(invsub+j*ldq),  1) )  
                     call IGRAPHARPACKDSCAL ( ncv, one / temp, 
     &                      workl(invsub+(j-1)*ldq), 1 )
                     call IGRAPHARPACKDSCAL ( ncv, one / temp, 
...
c
 40         continue
c
            call IGRAPHARPACKDGEMV('T', ncv, nconv, one, workl(invsub),
...
c
                  if (iconj .eq. 0) then
                     workev(j) = IGRAPHARPACKDLAPY2(workev(j), workev(j+1))
                     workev(j+1) = workev(j)
                     iconj = 1
                  else
                     iconj = 0
...
c
            if (msglvl .gt. 2) then
               call IGRAPHARPACKDCOPY(ncv, workl(invsub+ncv-1), ldq,
     &                    workl(ihbds), 1)
               call IGRAPHARPACKDVOUT (logfil, ncv, workl(ihbds), ndigit,
     &              '_neupd: Last row of the eigenvector matrix for T')
               if (msglvl .gt. 3) then
                  call IGRAPHARPACKDMOUT (logfil, ncv, ncv, workl(invsub), ldq, 
...
c           %---------------------------------------%
c
            call IGRAPHARPACKDCOPY(nconv, workev, 1, workl(ihbds), 1)
c
c           %---------------------------------------------------------%
...
c     
            call IGRAPHARPACKDTRMM ('Right', 'Upper', 'No transpose', 'Non-unit',
...
c        | Place the Ritz values computed IGRAPHARPACKDNAUPD into DR and DI |
c        %------------------------------------------------------%
c
         call IGRAPHARPACKDCOPY (nconv, workl(ritzr), 1, dr, 1)
         call IGRAPHARPACKDCOPY (nconv, workl(ritzi), 1, di, 1)
         call IGRAPHARPACKDCOPY (nconv, workl(ritzr), 1, workl(iheigr), 1)
         call IGRAPHARPACKDCOPY (nconv, workl(ritzi), 1, workl(iheigi), 1)
         call IGRAPHARPACKDCOPY (nconv, workl(bounds), 1, workl(ihbds), 1)
      end if
c 
c     %------------------------------------------------%
...
c
         if (rvec) 
     &      call IGRAPHARPACKDSCAL (ncv, rnorm, workl(ihbds), 1)     
c     
      else 
c     
c        %---------------------------------------%
...
c
            do 50 k=1, ncv
               temp = IGRAPHARPACKDLAPY2( workl(iheigr+k-1), 
     &                        workl(iheigi+k-1) )
...
c
      if (type .eq. 'SHIFTI' .and. msglvl .gt. 1) then
         call IGRAPHARPACKDVOUT (logfil, nconv, dr, ndigit,
     &   '_neupd: Untransformed real part of the Ritz valuess.')
         call IGRAPHARPACKDVOUT (logfil, nconv, di, ndigit,
     &   '_neupd: Untransformed imag part of the Ritz valuess.')
         call IGRAPHARPACKDVOUT (logfil, nconv, workl(ihbds), ndigit,
...
c
     ...
     &                      workl(iheigr+j-1)
               ...
               temp = IGRAPHARPACKDLAPY2( workl(iheigr+j-1), workl(iheigi+j-1) )
               workev(j) = ( workl(invsub+(j-1)*ldq+ncv-1) * 
     &                       workl(iheigr+j-1) +
...
c        %---------------------------------------%
c
         call IGRAPHARPACKDGER (n, nconv, one, resid, 1, workev, 1, z, ldz)
c
      end if
c
 9000 continue
