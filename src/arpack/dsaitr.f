...
c
c  where OP and B are as in IGRAPHARPACKDSAUPD.  The B-norm of r_{k+p} is also
c  computed and returned.
c
c\Usage:
c  call IGRAPHARPACKDSAITR
...
c          semi-inner product for the operator OP.  See IGRAPHARPACKDSAUPD.
...
c          and the main diagonal in the IGRAPHARPACKSECOND column.
c
c  LDH     Integer.  (INPUT)
...
c        p_{j} = p_{j}/betaj
c     3) r_{j} = OP*v_{j} where OP is defined as in IGRAPHARPACKDSAUPD
...
c     %----------------------%
c
      external   IGRAPHARPACKDAXPY, IGRAPHARPACKDCOPY, IGRAPHARPACKDSCAL, IGRAPHARPACKDGEMV, IGRAPHARPACKDGETV0, IGRAPHARPACKDVOUT, IGRAPHARPACKDMOUT,
     &           IGRAPHARPACKDLASCL, IGRAPHARPACKIVOUT, IGRAPHARPACKSECOND
c
c     %--------------------%
...
c        %--------------------------------%
c
         safmin = IGRAPHARPACKDLAMCH('safmin')
      end if
c
      if (ido .eq. 0) then
c 
...
c           %----------------------------------%
c
            wnorm = IGRAPHARPACKDDOT (n, resid, 1, workd(ivj), 1)
...
c
         if (mode .ne. 2 ) then
            call IGRAPHARPACKDGEMV('T', n, j, one, v, ldv, workd(ipj), 1, zero, 
     &                  workd(irj), 1)
         else if (mode .eq. 2) then
            call IGRAPHARPACKDGEMV('T', n, j, one, v, ldv, workd(ivj), 1, zero, 
     &                  workd(irj), 1)
         end if
c
c        %--------------------------------------%
...
c        %--------------------------------------%
c
         call IGRAPHARPACKDGEMV('N', n, j, -one, v, ldv, workd(irj), 1, one, 
     &               resid, 1)
c
c        %--------------------------------------%
...
c
         if (h(j,1) .lt. zero) then
            h(j,1) = -h(j,1)
            if ( j .lt. k+np) then 
               call IGRAPHARPACKDSCAL(n, -one, v(1,j+1), 1)
            else
               call IGRAPHARPACKDSCAL(n, -one, resid, 1)
            end if
         end if
c 
c        %------------------------------------%
...
c
            if (msglvl .gt. 1) then
               call IGRAPHARPACKDVOUT (logfil, k+np, h(1,2), ndigit, 
     &         '_saitr: main diagonal of matrix H of step K+NP.')
               if (k+np .gt. 1) then
               call IGRAPHARPACKDVOUT (logfil, k+np-1, h(2,1), ndigit, 
