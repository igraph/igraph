...
c     %----------------------%
c
      external   igraphdaxpy, igraphdcopy, igraphdscal, igraphdgemv, 
     &           igraphdgetv0, igraphdvout, igraphdmout,
     &           igraphdlascl, igraphivout, igraphsecond
c
c     %--------------------%
...
c
      Double precision
     &           igraphddot, igraphdnrm2, igraphdlamch
      external   igraphddot, igraphdnrm2, igraphdlamch
c
c     %-----------------%
c     | Data statements |
c     %-----------------%
...
c        %---------------------------------------------------------%
c
         call igraphdcopy (n, resid, 1, v(1,j), 1)
         if (rnorm .ge. safmin) then
             temp1 = one / rnorm
             call igraphdscal (n, temp1, v(1,j), 1)
             call igraphdscal (n, temp1, workd(ipj), 1)
         else
c
c            %-----------------------------------------%
...
c            %-----------------------------------------%
c
             call igraphdlascl ('General', i, i, rnorm, one, n, 1, 
     &                    v(1,j), n, infol)
             call igraphdlascl ('General', i, i, rnorm, one, n, 1, 
     &                    workd(ipj), n, infol)
         end if
...
c
         ...
         call igraphdcopy (n, v(1,j), 1, workd(ivj), 1)
         ipntr(1) = ivj
         ipntr(2) = irj
         ipntr(3) = ipj
         ido = 1
...
c        %------------------------------------------%
c
         call igraphdcopy (n, workd(irj), 1, resid, 1)
c 
c        %-------------------------------------------%
...
c 
            go to 9000
              ...
              call igraphdcopy(n, resid, 1 , workd(ipj), 1)
         end if
   60    continue
c 
c        %-----------------------------------%
...
c           %----------------------------------%
c
            wnorm = igraphddot (n, resid, 1, workd(ivj), 1)
            wnorm = sqrt(abs(wnorm))
         else if (bmat .eq. 'G') then         
            wnorm = igraphddot (n, resid, 1, workd(ipj), 1)
            wnorm = sqrt(abs(wnorm))
         else if (bmat .eq. 'I') then
            wnorm = igraphdnrm2(n, resid, 1)
         end if
c
c        %-----------------------------------------%
...
c
         if (mode .ne. 2 ) then
            call igraphdgemv('T', n, j, one, v, ldv, workd(ipj), 1, 
     &                  zero, workd(irj), 1)
         else if (mode .eq. 2) then
            call igraphdgemv('T', n, j, one, v, ldv, workd(ivj), 1, 
     &                  zero, workd(irj), 1)
         end if
c
c        %--------------------------------------%
...
c        %--------------------------------------%
c
         call igraphdgemv('N', n, j, -one, v, ldv, workd(irj), 1, one, 
     &               resid, 1)
c
c        %--------------------------------------%
...
c 
         ...
         if (bmat .eq. 'G') then
            nbx = nbx + 1
            call igraphdcopy (n, resid, 1, workd(irj), 1)
            ipntr(1) = irj
            ipntr(2) = ipj
            ido = 2
c 
...
c
         if (bmat .eq. 'G') then         
            rnorm = igraphddot (n, resid, 1, workd(ipj), 1)
            rnorm = sqrt(abs(rnorm))
         else if (bmat .eq. 'I') then
            rnorm = igraphdnrm2(n, resid, 1)
         end if
c
c        %-----------------------------------------------------------%
...
c        %----------------------------------------------%
c
         call igraphdgemv ('N', n, j, -one, v, ldv, workd(irj), 1, 
     &               one, resid, 1)
c
...
c 
         if (bmat .eq. 'G') then         
             rnorm1 = igraphddot (n, resid, 1, workd(ipj), 1)
             rnorm1 = sqrt(abs(rnorm1))
         else if (bmat .eq. 'I') then
             rnorm1 = igraphdnrm2(n, resid, 1)
         end if
c
...
c
         if (h(j,1) .lt. zero) then
            h(j,1) = -h(j,1)
            if ( j .lt. k+np) then 
               call igraphdscal(n, -one, v(1,j+1), 1)
            else
               call igraphdscal(n, -one, resid, 1)
            end if
         end if
c 
c        %------------------------------------%
