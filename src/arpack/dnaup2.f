c\BeginDoc
c
c\Name: IGRAPHARPACKDNAUP2
c
c\Description: 
c  Intermediate level interface called by IGRAPHARPACKDNAUPD.
c
c\Usage:
c  call IGRAPHARPACKDNAUP2
...
c\Arguments
c
c  IDO, BMAT, N, WHICH, NEV, TOL, RESID: same as defined in IGRAPHARPACKDNAUPD.
c  MODE, ISHIFT, MXITER: see the definition of IPARAM in IGRAPHARPACKDNAUPD.
c
...
c          together. The IGRAPHARPACKSECOND, is that a leading block of the current
...
c          listed in the same order as returned from IGRAPHARPACKDNEIGH.
c
...
c          See Data Distribution Note in IGRAPHARPACKDNAUPD.
c
...
c
c\Routines called:
c     IGRAPHARPACKDGETV0  ARPACK initial vector generation routine. 
c     IGRAPHARPACKDNAITR  ARPACK Arnoldi factorization routine.
c     IGRAPHARPACKDNAPPS  ARPACK application of implicit shifts routine.
c     IGRAPHARPACKDNCONV  ARPACK convergence of Ritz values routine.
c     IGRAPHARPACKDNEIGH  ARPACK compute Ritz values and error bounds routine.
c     IGRAPHARPACKDNGETS  ARPACK reorder Ritz values and error bounds routine.
c     IGRAPHARPACKDSORTC  ARPACK sorting routine.
c     IGRAPHARPACKIVOUT   ARPACK utility routine that prints integers.
c     IGRAPHARPACKSECOND  ARPACK utility routine for timing.
...
c     IGRAPHARPACKDSWAP   Level 1 BLAS that swaps two vectors.
c
c\Author
...
c     %----------------------%
c
      external   IGRAPHARPACKDCOPY, IGRAPHARPACKDGETV0, IGRAPHARPACKDNAITR, IGRAPHARPACKDNCONV, IGRAPHARPACKDNEIGH, IGRAPHARPACKDNGETS, IGRAPHARPACKDNAPPS,
     &           IGRAPHARPACKDVOUT, IGRAPHARPACKIVOUT, IGRAPHARPACKSECOND
c
c     %--------------------%
...
c
      Double precision
     &           IGRAPHARPACKDDOT, IGRAPHARPACKDNRM2, IGRAPHARPACKDLAPY2, IGRAPHARPACKDLAMCH
      external   IGRAPHARPACKDDOT, IGRAPHARPACKDNRM2, IGRAPHARPACKDLAPY2, IGRAPHARPACKDLAMCH
c
c     %---------------------%
...
c
      if (getv0) then
         call IGRAPHARPACKDGETV0 (ido, bmat, 1, initv, n, 1, v, ldv, resid, rnorm,
     &                ipntr, workd, info)
c
...
c     %----------------------------------------------------------%
c
      call IGRAPHARPACKDNAITR (ido, bmat, n, 0, nev, mode, resid, rnorm, v, ldv, 
...
c
         if (msglvl .gt. 0) then
            call IGRAPHARPACKIVOUT (logfil, 1, iter, ndigit, 
...
c        | to the shift application routine IGRAPHARPACKDNAPPS.                  |
c        %-----------------------------------------------------------%
c
         np  = kplusp - nev
c
         if (msglvl .gt. 1) then
            call IGRAPHARPACKIVOUT (logfil, 1, nev, ndigit, 
     &     '_naup2: The length of the current Arnoldi factorization')
            call IGRAPHARPACKIVOUT (logfil, 1, np, ndigit, 
...
c
   ...
   20    continue
         update = .true.
c
         call IGRAPHARPACKDNAITR (ido, bmat, n, nev, np, mode, resid, rnorm, v, ldv,
     &                h, ldh, ipntr, workd, info)
c 
c        %---------------------------------------------------%
c        | ido .ne. 99 implies use of reverse communication  |
...
c
         if (msglvl .gt. 1) then
            call IGRAPHARPACKDVOUT (logfil, 1, rnorm, ndigit, 
...
c        %--------------------------------------------------------%
c
         call IGRAPHARPACKDNEIGH (rnorm, kplusp, h, ldh, ritzr, ritzi, bounds,
     &                q, ldq, workl, ierr)
c
         if (ierr .ne. 0) then
            info = -8
...
c        | bounds obtained from IGRAPHARPACKDNEIGH.                       |
c        %----------------------------------------------------%
c
         call IGRAPHARPACKDCOPY(kplusp, ritzr, 1, workl(kplusp**2+1), 1)
         call IGRAPHARPACKDCOPY(kplusp, ritzi, 1, workl(kplusp**2+kplusp+1), 1)
         call IGRAPHARPACKDCOPY(kplusp, bounds, 1, workl(kplusp**2+2*kplusp+1), 1)
c
c        %---------------------------------------------------%
...
c        | NOTE: The last two arguments of IGRAPHARPACKDNGETS are no     |
...
c
         ...
         np = np0
         numcnv = nev
         call IGRAPHARPACKDNGETS (ishift, which, nev, np, ritzr, ritzi, 
...
c        %-------------------%
c
         call IGRAPHARPACKDCOPY (nev, bounds(np+1), 1, workl(2*np+1), 1)
         call IGRAPHARPACKDNCONV (nev, ritzr(np+1), ritzi(np+1), workl(2*np+1), 
     &        tol, nconv)
c 
         if (msglvl .gt. 2) then
            kp(1) = nev
            ...
            kp(3) = numcnv
            kp(4) = nconv
            call IGRAPHARPACKIVOUT (logfil, 4, kp, ndigit, 
     &                  '_naup2: NEV, NP, NUMCNV, NCONV are')
            call IGRAPHARPACKDVOUT (logfil, kplusp, ritzr, ndigit,
     &           '_naup2: Real part of the eigenvalues of H')
            call IGRAPHARPACKDVOUT (logfil, kplusp, ritzi, ndigit,
     &           '_naup2: Imaginary part of the eigenvalues of H')
            call IGRAPHARPACKDVOUT (logfil, kplusp, bounds, ndigit, 
...
c
            if (msglvl .gt. 4) then
               call IGRAPHARPACKDVOUT(logfil, kplusp, workl(kplusp**2+1), ndigit,
     &             '_naup2: Real part of the eig computed by _neigh:')
               call IGRAPHARPACKDVOUT(logfil, kplusp, workl(kplusp**2+kplusp+1),
     &                     ndigit,
     &             '_naup2: Imag part of the eig computed by _neigh:')
               call IGRAPHARPACKDVOUT(logfil, kplusp, workl(kplusp**2+kplusp*2+1),
     &                     ndigit,
...
c
c           %----------------------------------------------%
c           | To be consistent with IGRAPHARPACKDNGETS, we first do a  |
c           | pre-processing sort in order to keep complex |
c           | conjugate pairs together.  This is similar   |
c           | to the pre-processing sort used in IGRAPHARPACKDNGETS    |
...
c
            call IGRAPHARPACKDSORTC (wprime, .true., kplusp, ritzr, ritzi, bounds)
c
c           %----------------------------------------------%
...
c
            do 35 j = 1, nev0
                temp = max(eps23,IGRAPHARPACKDLAPY2(ritzr(j),
     &                                   ritzi(j)))
...
c
            wprime = 'LR'
            call IGRAPHARPACKDSORTC(wprime, .true., nev0, bounds, ritzr, ritzi)
c
c           %----------------------------------------------%
...
c           %------------------------------------------------%
c
            call IGRAPHARPACKDSORTC(which, .true., nconv, ritzr, ritzi, bounds)
c
            if (msglvl .gt. 1) then
               call IGRAPHARPACKDVOUT (logfil, kplusp, ritzr, ndigit,
...
c     
     ...
     &           '_naup2: no. of "converged" Ritz values at this iter.')
            if (msglvl .gt. 1) then
               kp(1) = nev
               kp(2) = np
               call IGRAPHARPACKIVOUT (logfil, 2, kp, ndigit, 
     &              '_naup2: NEV and NP are')
               call IGRAPHARPACKDVOUT (logfil, nev, ritzr(np+1), ndigit,
     &              '_naup2: "wanted" Ritz values -- real part')
               call IGRAPHARPACKDVOUT (logfil, nev, ritzi(np+1), ndigit,
     &              '_naup2: "wanted" Ritz values -- imag part')
               call IGRAPHARPACKDVOUT (logfil, nev, bounds(np+1), ndigit,
...
c            %----------------------------------%
c
             call IGRAPHARPACKDCOPY (np, workl,       1, ritzr, 1)
             call IGRAPHARPACKDCOPY (np, workl(np+1), 1, ritzi, 1)
         end if
c
         if (msglvl .gt. 2) then 
            call IGRAPHARPACKIVOUT (logfil, 1, np, ndigit, 
     &                  '_naup2: The number of shifts to apply ')
            call IGRAPHARPACKDVOUT (logfil, np, ritzr, ndigit,
     &                  '_naup2: Real part of the shifts')
            call IGRAPHARPACKDVOUT (logfil, np, ritzi, ndigit,
     &                  '_naup2: Imaginary part of the shifts')
            if ( ishift .eq. 1 ) 
     &          call IGRAPHARPACKDVOUT (logfil, np, bounds, ndigit,
...
c        %---------------------------------------------------------%
c
         call IGRAPHARPACKDNAPPS (n, nev, np, ritzr, ritzi, v, ldv, 
     &                h, ldh, resid, q, ldq, workl, workd)
c
c        %---------------------------------------------%
c        | Compute the B-norm of the updated residual. |
c        | Keep B*RESID in WORKD(1:N) to be used in    |
c        | the first step of the next call to IGRAPHARPACKDNAITR.  |
c        %---------------------------------------------%
c
         cnorm = .true.
         call IGRAPHARPACKSECOND (t2)
         if (bmat .eq. 'G') then
            nbx = nbx + 1
            call IGRAPHARPACKDCOPY (n, resid, 1, workd(n+1), 1)
            ipntr(1) = n + 1
            ipntr(2) = 1
            ido = 2
c 
...
c 
            ...
            call IGRAPHARPACKDCOPY (n, resid, 1, workd, 1)
         end if
c 
  100    continue
c 
...
c
         if (bmat .eq. 'G') then
            call IGRAPHARPACKSECOND (t3)
            tmvbx = tmvbx + (t3 - t2)
         end if
c 
         if (bmat .eq. 'G') then         
            rnorm = IGRAPHARPACKDDOT (n, resid, 1, workd, 1)
            rnorm = sqrt(abs(rnorm))
         else if (bmat .eq. 'I') then
            rnorm = IGRAPHARPACKDNRM2(n, resid, 1)
         end if
         cnorm = .false.
c
         if (msglvl .gt. 2) then
     ...
     &      '_naup2: B-norm of residual for compressed factorization')
            call IGRAPHARPACKDMOUT (logfil, nev, nev, h, ldh, ndigit,
...
c     %------------%
c
      call IGRAPHARPACKSECOND (t1)
      tnaup2 = t1 - t0
c     
 9000 continue
c
c     %---------------%
c     | End of IGRAPHARPACKDNAUP2 |
c     %---------------%
c
      return
      end
