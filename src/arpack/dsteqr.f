      SUBROUTINE IGRAPHARPACKDSTEQR( COMPZ, N, D, E, Z, LDZ, WORK, INFO )
*
...
*  =======
*
*  IGRAPHARPACKDSTEQR computes all eigenvalues and, optionally, eigenvectors of a
...
*     .. External Functions ..
      ...
      DOUBLE PRECISION   IGRAPHARPACKDLAMCH, IGRAPHARPACKDLANST, IGRAPHARPACKDLAPY2
...
*
      INFO = 0
*
      IF( IGRAPHARPACKLSAME( COMPZ, 'N' ) ) THEN
         ICOMPZ = 0
      ELSE IF( IGRAPHARPACKLSAME( COMPZ, 'V' ) ) THEN
         ICOMPZ = 1
      ELSE IF( IGRAPHARPACKLSAME( COMPZ, 'I' ) ) THEN
         ICOMPZ = 2
      ELSE
         ICOMPZ = -1
      END IF
     ...
     $         N ) ) ) THEN
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL IGRAPHARPACKXERBLA( 'IGRAPHARPACKDSTEQR', -INFO )
         RETURN
      END IF
*
...
*
      IF( ICOMPZ.EQ.2 )
     $   CALL IGRAPHARPACKDLASET( 'Full', N, N, ZERO, ONE, Z, LDZ )
*
      NMAXIT = N*MAXIT
      JTOT = 0
*
...
*
   60    CONTINUE
     ...
     $      GO TO 80
*
*        If remaining matrix is 2-by-2, use IGRAPHARPACKDLAE2 or SLAEV2
...
*
         IF( M.EQ.L+1 ) THEN
            IF( ICOMPZ.GT.0 ) THEN
               ...
               WORK( L ) = C
               WORK( N-1+L ) = S
               CALL IGRAPHARPACKDLASR( 'R', 'V', 'B', N, 2, WORK( L ),
...
*
         IF( ICOMPZ.GT.0 ) THEN
            MM = M - L + 1
            CALL IGRAPHARPACKDLASR( 'R', 'V', 'B', N, MM, WORK( L ), WORK( N-1+L ),
     $                  Z( 1, L ), LDZ )
         END IF
*
         D( L ) = D( L ) - P
...
*
         IF( M.EQ.L-1 ) THEN
            IF( ICOMPZ.GT.0 ) THEN
               ...
               WORK( M ) = C
               WORK( N-1+M ) = S
               CALL IGRAPHARPACKDLASR( 'R', 'V', 'F', N, 2, WORK( M ),
...
*
         IF( ICOMPZ.GT.0 ) THEN
            MM = L - M + 1
            CALL IGRAPHARPACKDLASR( 'R', 'V', 'F', N, MM, WORK( M ), WORK( N-1+M ),
     $                  Z( 1, M ), LDZ )
         END IF
*
         D( L ) = D( L ) - P
...
*
  ...
  170       CONTINUE
            IF( K.NE.I ) THEN
               D( K ) = D( I )
               D( I ) = P
               CALL IGRAPHARPACKDSWAP( N, Z( 1, I ), 1, Z( 1, K ), 1 )
            END IF
  180    CONTINUE
      END IF
*
