...
c     %----------------------%
c
      external   igraphdswap, igraphdcopy, igraphdsortr, igraphsecond
c
c     %---------------------%
...
c
         ...
         kevd2 = kev / 2 
         if ( kev .gt. 1 ) then
            call igraphdswap ( min(kevd2,np), ritz, 1, 
     &                   ritz( max(kevd2,np)+1 ), 1)
            call igraphdswap ( min(kevd2,np), bounds, 1, 
...
c     
         ...
         call igraphdcopy (np, ritz, 1, shifts, 1)
      end if
c 
