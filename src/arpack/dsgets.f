...
c
c\Routines called:
c     IGRAPHARPACKDSORTR  ARPACK utility sorting routine.
...
c-----------------------------------------------------------------------
c
      subroutine IGRAPHARPACKDSGETS ( ishift, which, kev, np, ritz, bounds, shifts )
c
c     %----------------------------------------------------%
...
c     %----------------------%
c
      external   IGRAPHARPACKDSWAP, IGRAPHARPACKDCOPY, IGRAPHARPACKDSORTR, IGRAPHARPACKSECOND
c
c     %---------------------%
...
c        %-----------------------------------------------------%
c
         call IGRAPHARPACKDSORTR ('LA', .true., kev+np, ritz, bounds)
         kevd2 = kev / 2 
         if ( kev .gt. 1 ) then
            call IGRAPHARPACKDSWAP ( min(kevd2,np), ritz, 1, 
     &                   ritz( max(kevd2,np)+1 ), 1)
            call IGRAPHARPACKDSWAP ( min(kevd2,np), bounds, 1, 
...
c        %----------------------------------------------------%
c
         call IGRAPHARPACKDSORTR (which, .true., kev+np, ritz, bounds)
      end if
c
...
c        | are applied in subroutine IGRAPHARPACKDSAPPS.                     |
c        %-------------------------------------------------------%
c     
         call IGRAPHARPACKDSORTR ('SM', .true., np, bounds, ritz)
         call IGRAPHARPACKDCOPY (np, ritz, 1, shifts, 1)
      end if
c 
...
c
      if (msglvl .gt. 0) then
         call IGRAPHARPACKIVOUT (logfil, 1, kev, ndigit, '_sgets: KEV is')
         call IGRAPHARPACKIVOUT (logfil, 1, np, ndigit, '_sgets: NP is')
         call IGRAPHARPACKDVOUT (logfil, kev+np, ritz, ndigit,
     &        '_sgets: Eigenvalues of current H matrix')
         call IGRAPHARPACKDVOUT (logfil, kev+np, bounds, ndigit, 
