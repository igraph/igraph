...
c          Error exit flag from IGRAPHARPACKDLAQRB or IGRAPHARPACKDTREVC.
c
c\EndDoc
c
c-----------------------------------------------------------------------
...
c
c\Routines called:
c     IGRAPHARPACKDLAQRB  ARPACK routine to compute the real Schur form of an
...
c-----------------------------------------------------------------------
c
      subroutine IGRAPHARPACKDNEIGH (rnorm, n, h, ldh, ritzr, ritzi, bounds, 
     &                   q, ldq, workl, ierr)
c
c     %----------------------------------------------------%
...
c     %----------------------%
c
      external   IGRAPHARPACKDCOPY, IGRAPHARPACKDLACPY, IGRAPHARPACKDLAQRB, IGRAPHARPACKDTREVC, IGRAPHARPACKDVOUT, IGRAPHARPACKSECOND
c
c     %--------------------%
...
c
      Double precision
     &           IGRAPHARPACKDLAPY2, IGRAPHARPACKDNRM2
      external   IGRAPHARPACKDLAPY2, IGRAPHARPACKDNRM2
c
c     %---------------------%
...
c 
      if (msglvl .gt. 2) then
          call IGRAPHARPACKDMOUT (logfil, n, n, h, ldh, ndigit, 
...
c     | IGRAPHARPACKDLAQRB returns the full Schur form of H in WORKL(1:N**2)  |
c     | and the last components of the Schur vectors in BOUNDS.   |
c     %-----------------------------------------------------------%
c
      call IGRAPHARPACKDLACPY ('All', n, n, h, ldh, workl, n)
      call IGRAPHARPACKDLAQRB (.true., n, 1, n, workl, n, ritzr, ritzi, bounds,
     &             ierr)
      if (ierr .ne. 0) go to 9000
c
      if (msglvl .gt. 1) then
         call IGRAPHARPACKDVOUT (logfil, n, bounds, ndigit,
...
c     %-----------------------------------------------------------%
c
      call IGRAPHARPACKDTREVC ('R', 'A', select, n, workl, n, vl, n, q, ldq,
...
c           %----------------------%
c    
            temp = IGRAPHARPACKDNRM2( n, q(1,i), 1 )
            call IGRAPHARPACKDSCAL ( n, one / temp, q(1,i), 1 )
         else
c
c           %-------------------------------------------%
...
c
            if (iconj .eq. 0) then
               temp = IGRAPHARPACKDLAPY2( IGRAPHARPACKDNRM2( n, q(1,i), 1 ), 
     &                        IGRAPHARPACKDNRM2( n, q(1,i+1), 1 ) )
               call IGRAPHARPACKDSCAL ( n, one / temp, q(1,i), 1 )
               call IGRAPHARPACKDSCAL ( n, one / temp, q(1,i+1), 1 )
               iconj = 1
            else
               iconj = 0
            end if
   ...
   10 continue
c
      call IGRAPHARPACKDGEMV ('T', n, n, one, q, ldq, bounds, 1, zero, workl, 1)
c
      if (msglvl .gt. 1) then
         call IGRAPHARPACKDVOUT (logfil, n, workl, ndigit,
...
c
            if (iconj .eq. 0) then
               bounds(i) = rnorm * IGRAPHARPACKDLAPY2( workl(i), workl(i+1) )
               bounds(i+1) = bounds(i)
               iconj = 1
            else
               iconj = 0
...
c
      if (msglvl .gt. 2) then
         call IGRAPHARPACKDVOUT (logfil, n, ritzr, ndigit,
     &              '_neigh: Real part of the eigenvalues of H')
         call IGRAPHARPACKDVOUT (logfil, n, ritzi, ndigit,
