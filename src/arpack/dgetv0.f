...
c     %----------------------%
c
      external   igraphdlarnv, igraphdvout, igraphdcopy, igraphdgemv, 
     &           igraphsecond
c
c     %--------------------%
...
c
      Double precision
     &           igraphddot, igraphdnrm2
      external   igraphddot, igraphdnrm2
c
c     %---------------------%
...
c
         if (.not.initv) then
            idist = 2
            call igraphdlarnv (idist, iseed, n, resid)
         end if
c 
c        %----------------------------------------------------------%
...
c
      ...
      if (bmat .eq. 'G') then
         nbx = nbx + 1
         call igraphdcopy (n, workd(n+1), 1, resid, 1)
         ipntr(1) = n + 1
         ipntr(2) = 1
         ido = 2
         go to 9000
...
c 
      first = .FALSE.
      if (bmat .eq. 'G') then
          rnorm0 = igraphddot (n, resid, 1, workd, 1)
          rnorm0 = sqrt(abs(rnorm0))
      else if (bmat .eq. 'I') then
           rnorm0 = igraphdnrm2(n, resid, 1)
      end if
      rnorm  = rnorm0
c
c     %---------------------------------------------%
...
c
   ...
   30 continue
c
      call igraphdgemv ('T', n, j-1, one, v, ldv, workd, 1, 
     &            zero, workd(n+1), 1)
      call igraphdgemv ('N', n, j-1, -one, v, ldv, workd(n+1), 1, 
     &            one, resid, 1)
c 
c     %----------------------------------------------------------%
