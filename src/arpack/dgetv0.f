...
c          ITRY counts the number of times that IGRAPHARPACKDGETV0 is called.  
c          It should be set to 1 on the initial call to IGRAPHARPACKDGETV0.
c
...
c     IGRAPHARPACKDVOUT   ARPACK utility routine for vector output.
c     IGRAPHARPACKDLARNV  LAPACK routine for generating a random vector.
...
c     %----------------------%
c
      external   IGRAPHARPACKDLARNV, IGRAPHARPACKDVOUT, IGRAPHARPACKDCOPY, IGRAPHARPACKDGEMV, IGRAPHARPACKSECOND
c
c     %--------------------%
...
c
      Double precision
     &           IGRAPHARPACKDDOT, IGRAPHARPACKDNRM2
      external   IGRAPHARPACKDDOT, IGRAPHARPACKDNRM2
c
c     %---------------------%
...
c
         if (.not.initv) then
            idist = 2
            call IGRAPHARPACKDLARNV (idist, iseed, n, resid)
         end if
c 
c        %----------------------------------------------------------%
...
c
      ...
      if (bmat .eq. 'G') then
         nbx = nbx + 1
         call IGRAPHARPACKDCOPY (n, workd(n+1), 1, resid, 1)
         ipntr(1) = n + 1
         ipntr(2) = 1
         ido = 2
         go to 9000
...
c 
      first = .FALSE.
      if (bmat .eq. 'G') then
          rnorm0 = IGRAPHARPACKDDOT (n, resid, 1, workd, 1)
          rnorm0 = sqrt(abs(rnorm0))
      else if (bmat .eq. 'I') then
           rnorm0 = IGRAPHARPACKDNRM2(n, resid, 1)
      end if
      rnorm  = rnorm0
c
c     %---------------------------------------------%
...
c
   ...
   30 continue
c
      call IGRAPHARPACKDGEMV ('T', n, j-1, one, v, ldv, workd, 1, 
     &            zero, workd(n+1), 1)
      call IGRAPHARPACKDGEMV ('N', n, j-1, -one, v, ldv, workd(n+1), 1, 
     &            one, resid, 1)
c 
c     %----------------------------------------------------------%
...
c
      if (msglvl .gt. 2) then
          call IGRAPHARPACKDVOUT (logfil, 1, rnorm0, ndigit, 
...
c
     ...
     &        '_getv0: B-norm of initial / restarted starting vector')
      ...
      if (msglvl .gt. 2) then
         call IGRAPHARPACKDVOUT (logfil, n, resid, ndigit,
