...
c     .. external functions ..
      logical            igraphlsame
      Double precision
     &                   igraphdlamch, igraphdlanst, igraphdlapy2
      external           igraphlsame, igraphdlamch, igraphdlanst, 
     &                   igraphdlapy2
c     ..
c     .. external subroutines ..
      external           igraphdlae2, igraphdlaev2, igraphdlartg, 
     &                   igraphdlascl, igraphdlaset, igraphdlasr,
     &                   igraphdlasrt, igraphdswap, igraphxerbla
c     ..
...
c
      anorm = igraphdlanst( 'i', lend-l+1, d( l ), e( l ) )
      iscale = 0
      if( anorm.eq.zero )
     $   go to 10
      if( anorm.gt.ssfmax ) then
         iscale = 1
         call igraphdlascl( 'g', 0, 0, anorm, ssfmax, lend-l+1, 1,
     $                d( l ), n, info )
         call igraphdlascl( 'g', 0, 0, anorm, ssfmax, lend-l, 1, e( l ),
     $                n, info )
      else if( anorm.lt.ssfmin ) then
         iscale = 2
         call igraphdlascl( 'g', 0, 0, anorm, ssfmin, lend-l+1, 1,
     $                d( l ), n, info )
         call igraphdlascl( 'g', 0, 0, anorm, ssfmin, lend-l, 1, e( l ),
     $                n, info )
      end if
c
...
c
         if( m.eq.l+1 ) then
            if( icompz.gt.0 ) then
               call igraphdlaev2( d( l ), e( l ), d( l+1 ), rt1, rt2,
     $                            c, s )
               work( l ) = c
               work( n-1+l ) = s
...
c              *************************************
               ...
               call igraphdlae2( d( l ), e( l ), d( l+1 ), rt1, rt2 )
            end if
            d( l ) = rt1
            d( l+1 ) = rt2
            e( l ) = zero
...
c             *** New starting with version 2.5 ***
c
              call igraphdlasr( 'r', 'v', 'b', 1, mm, work( l ), 
     &                    work( n-1+l ), z( l ), 1 )
c             *************************************                             
         end if
c
...
c
         if( m.eq.l-1 ) then
            if( icompz.gt.0 ) then
               call igraphdlaev2( d( l-1 ), e( l-1 ), d( l ), rt1, 
     $                            rt2, c, s )
c$$$               work( m ) = c
c$$$               work( n-1+m ) = s
...
c               ************************************* 
               ...
               call igraphdlae2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2 )
            end if
            d( l-1 ) = rt1
            d( l ) = rt2
            e( l-1 ) = zero
...
c           *** New starting with version 2.5 ***
c
            call igraphdlasr( 'r', 'v', 'f', 1, mm, work( m ), 
     &                  work( n-1+m ), z( m ), 1 )
c           *************************************                             
         end if
c
...
c
  140 continue
      if( iscale.eq.1 ) then
         call igraphdlascl( 'g', 0, 0, ssfmax, anorm, lendsv-lsv+1, 1,
     $                d( lsv ), n, info )
         call igraphdlascl( 'g', 0, 0, ssfmax, anorm, lendsv-lsv, 1, 
     $                e( lsv ), n, info )
      else if( iscale.eq.2 ) then
         call igraphdlascl( 'g', 0, 0, ssfmin, anorm, lendsv-lsv+1, 1,
     $                d( lsv ), n, info )
         call igraphdlascl( 'g', 0, 0, ssfmin, anorm, lendsv-lsv, 1, 
     $                e( lsv ), n, info )
      end if
c
