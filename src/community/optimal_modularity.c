...
igraph_long_t igraph_community_optimal_modularity(const igraph_t *graph,
...
#define IDX(a,b) ((b)*((b)+1)/2+(a))
    ...
    for (i = 0; i < no_of_nodes; i++) {
        for (j = i + 1; j < no_of_nodes; j++) {
            ...
            for (k = j + 1; k < no_of_nodes; k++) {
                igraph_long_t newrow = glp_add_rows(ip, 3);
    ...
    if (membership) {
        igraph_long_t comm = 0;   /* id of the last community that was found */
        IGRAPH_CHECK(igraph_vector_resize(membership, no_of_nodes));
        for (i = 0; i < no_of_nodes; i++) {
            ...
            for (j = 0; j < i; j++) {
                igraph_long_t val = (igraph_long_t) glp_mip_col_val(ip, st + IDX(j, i));
                if (val == 1) {
