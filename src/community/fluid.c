...
igraph_error_t igraph_community_fluid_communities(const igraph_t *graph,
    ...
    if ((igraph_long_t) no_of_communities < 1) {
        IGRAPH_ERROR("'no_of_communities' must be greater than 0.", IGRAPH_EINVAL);
    }
    if ((igraph_long_t) no_of_communities > no_of_nodes) {
    ...
    IGRAPH_CHECK(igraph_vector_init(&density, (igraph_long_t) no_of_communities));
    IGRAPH_FINALLY(igraph_vector_destroy, &density);
    IGRAPH_CHECK(igraph_vector_long_init(&com_to_numvertices, (igraph_long_t) no_of_communities));
    IGRAPH_FINALLY(igraph_vector_long_destroy, &com_to_numvertices);
    ...
    for (i = 0; i < no_of_communities; i++) {
        ...
        VECTOR(*membership)[(igraph_long_t)VECTOR(node_order)[i]] = i + 1.0;
    ...
    IGRAPH_VECTOR_INIT_FINALLY(&dominant_labels, (igraph_long_t) no_of_communities);
    IGRAPH_VECTOR_INIT_FINALLY(&nonzero_labels, (igraph_long_t) no_of_communities);

    IGRAPH_CHECK(igraph_vector_init(&label_counters, (igraph_long_t) no_of_communities));
    ...
    while (running) {
        ...
        igraph_long_t v1, size, rand_idx;
        ...
        for (i = 0; i < no_of_nodes; i++) {
            ...
            v1 = (igraph_long_t) VECTOR(node_order)[i];
            /* Take into account same label in updating rule */
            kv1 = (igraph_long_t) VECTOR(*membership)[v1];
            max_count = 0.0;
            if (kv1 != 0) {
            ...
            size = igraph_vector_long_size(neis);
            for (j = 0; j < size; j++) {
                k = (igraph_long_t) VECTOR(*membership)[(igraph_long_t)VECTOR(*neis)[j]];
            ...
            if (!igraph_vector_empty(&dominant_labels)) {
                ...
                if (!same_label_in_dominant) {
                    ...
                    k = (igraph_long_t) VECTOR(dominant_labels)[rand_idx];
    ...
    igraph_vector_long_destroy(&com_to_numvertices);
