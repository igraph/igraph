...
static igraph_long_t igraph_i_clusters_weak(const igraph_t *graph, igraph_vector_t *membership,
                                  igraph_vector_t *csize, igraph_long_t *no);

static igraph_long_t igraph_i_clusters_strong(const igraph_t *graph, igraph_vector_t *membership,
                                    igraph_vector_t *csize, igraph_long_t *no);
...
static igraph_long_t igraph_i_clusters_weak(const igraph_t *graph, igraph_vector_t *membership,
    ...
    char *already_added;
    igraph_long_t first_node, act_cluster_size = 0, no_of_clusters = 1;
    ...
    for (first_node = 0; first_node < no_of_nodes; ++first_node) {
        ...
        while ( !igraph_dqueue_empty(&q) ) {
            ...
            IGRAPH_CHECK(igraph_neighbors(graph, &neis,
                                          (igraph_long_t) act_node, IGRAPH_ALL));
            for (i = 0; i < igraph_vector_size(&neis); i++) {
                igraph_long_t neighbor = (igraph_long_t) VECTOR(neis)[i];
    ...
    if (no) {
        *no = (igraph_long_t) no_of_clusters - 1;
    }
...
static igraph_long_t igraph_i_clusters_strong(const igraph_t *graph, igraph_vector_t *membership,
    ...
    igraph_long_t i, n, num_seen;
    igraph_dqueue_t q = IGRAPH_DQUEUE_NULL;

    igraph_long_t no_of_clusters = 1;
    igraph_long_t act_cluster_size;

    igraph_vector_t out = IGRAPH_VECTOR_NULL;
    const igraph_vector_long_t* tmp;
    ...
    for (i = 0; i < no_of_nodes; i++) {
        ...
        if (VECTOR(next_nei)[i] > igraph_vector_long_size(tmp)) {
            continue;
        }

        IGRAPH_CHECK(igraph_dqueue_push(&q, i));
        while (!igraph_dqueue_empty(&q)) {
            igraph_long_t act_node = (igraph_long_t) igraph_dqueue_back(&q);
            ...
            } else if (VECTOR(next_nei)[act_node] <= igraph_vector_long_size(tmp)) {
                /* we've already met this vertex but it has more children */
                igraph_long_t neighbor = (igraph_long_t) VECTOR(*tmp)[(igraph_long_t)
    ...
    while (!igraph_vector_empty(&out)) {
        igraph_long_t grandfather = (igraph_long_t) igraph_vector_pop_back(&out);
        ...
        while (!igraph_dqueue_empty(&q)) {
            igraph_long_t act_node = (igraph_long_t) igraph_dqueue_pop_back(&q);
            tmp = igraph_adjlist_get(&adjlist, act_node);
            n = igraph_vector_long_size(tmp);
            for (i = 0; i < n; i++) {
                igraph_long_t neighbor = (igraph_long_t) VECTOR(*tmp)[i];
                if (VECTOR(next_nei)[neighbor] != 0) {
                    continue;
                }
                IGRAPH_CHECK(igraph_dqueue_push(&q, neighbor));
...
igraph_long_t igraph_is_connected_weak(const igraph_t *graph, igraph_bool_t *res) {
    ...
    while ( !igraph_dqueue_empty(&q)) {
        ...
        IGRAPH_ALLOW_INTERRUPTION();
        IGRAPH_CHECK(igraph_neighbors(graph, &neis, (igraph_long_t) actnode,
                                      IGRAPH_ALL));
        for (i = 0; i < igraph_vector_size(&neis); i++) {
            igraph_long_t neighbor = (igraph_long_t) VECTOR(neis)[i];
...
static igraph_long_t igraph_i_decompose_weak(const igraph_t *graph,
                                   igraph_vector_ptr_t *components,
                                   igraph_long_t maxcompno, igraph_long_t minelements);

static igraph_long_t igraph_i_decompose_strong(const igraph_t *graph,
                                     igraph_vector_ptr_t *components,
                                     igraph_long_t maxcompno, igraph_long_t minelements);
...
static igraph_long_t igraph_i_decompose_weak(const igraph_t *graph,
    ...
    igraph_long_t actstart;
    igraph_long_t no_of_nodes = igraph_vcount(graph);
    igraph_long_t resco = 0;   /* number of graphs created so far */
    char *already_added;
    igraph_dqueue_t q;
    ...
    for (actstart = 0; resco < maxcompno && actstart < no_of_nodes; actstart++) {
        ...
        while (!igraph_dqueue_empty(&q) ) {
            ...
            igraph_long_t actvert = (igraph_long_t) igraph_dqueue_pop(&q);
            IGRAPH_CHECK(igraph_neighbors(graph, &neis, (igraph_long_t) actvert,
                                          IGRAPH_ALL));
...
static igraph_long_t igraph_i_decompose_strong(const igraph_t *graph,
    ...
    igraph_long_t no_of_clusters = 0;
    igraph_long_t act_cluster_size;
...
void igraph_i_free_vectorlist(igraph_vector_ptr_t *list) {
    igraph_long_t i, n = igraph_vector_ptr_size(list);
    for (i = 0; i < n; i++) {
...
igraph_long_t igraph_biconnected_components(const igraph_t *graph,
    ...
    igraph_vector_long_t *adjedges;
    igraph_stack_t path;
    igraph_vector_t edgestack;
    igraph_inclist_t inclist;
    igraph_long_t i, counter, rootdfs = 0;
    igraph_vector_long_t vertex_added;
    igraph_long_t comps = 0;
    ...
    for (i = 0; i < no_of_nodes; i++) {
        ...
        while (!igraph_stack_empty(&path)) {
            igraph_long_t n;
            igraph_long_t act = (igraph_long_t) igraph_stack_top(&path);
            igraph_long_t actnext = VECTOR(nextptr)[act];

            adjedges = igraph_inclist_get(&inclist, act);
            n = igraph_vector_long_size(adjedges);
            if (actnext < n) {
                /* Step down (maybe) */
                igraph_long_t edge = (igraph_long_t) VECTOR(*adjedges)[actnext];
                igraph_long_t nei = IGRAPH_OTHER(graph, edge, act);
                if (VECTOR(low)[nei] == 0) {
                    if (act == i) {
                        rootdfs++;
                    }
                ...
                if (!igraph_stack_empty(&path)) {
                    igraph_long_t prev = (igraph_long_t) igraph_stack_top(&path);
                    /* Update LOW value if needed */
                    ...
                    if (VECTOR(low)[act] >= VECTOR(num)[prev]) {
                        ...
                        if (tree_edges || mycomponents) {
                            ...
                            while (!igraph_vector_empty(&edgestack)) {
                                igraph_long_t e = (igraph_long_t) igraph_vector_pop_back(&edgestack);
                                igraph_long_t from = IGRAPH_FROM(graph, e);
                                igraph_long_t to = IGRAPH_TO(graph, e);
                                if (tree_edges) {
                            ...
                            if (component_edges) {
                                ...
                                igraph_long_t ii, no_vert = igraph_vector_size(nodes);
                                if (!vv) {
                                    IGRAPH_ERROR("Out of memory", IGRAPH_ENOMEM);
                                }
                                ...
                                for (ii = 0; ii < no_vert; ii++) {
                                    igraph_long_t vert = (igraph_long_t) VECTOR(*nodes)[ii];
                                    igraph_vector_long_t *edges = igraph_inclist_get(&inclist,
                                                                 vert);
                                    igraph_long_t j, nn = igraph_vector_long_size(edges);
                                    for (j = 0; j < nn; j++) {
                                        igraph_long_t e = (igraph_long_t) VECTOR(*edges)[j];
                                        igraph_long_t nei = IGRAPH_OTHER(graph, e, vert);
...
static igraph_long_t igraph_i_bridges_rec(
        const igraph_t *graph, const igraph_inclist_t *il, igraph_long_t u,
        igraph_long_t *time, igraph_vector_t *bridges, igraph_vector_bool_t *visited,
        igraph_vector_long_t *disc, igraph_vector_long_t *low, igraph_vector_long_t *incoming_edge)
{
    igraph_vector_long_t *incedges;
    igraph_long_t nc; /* neighbour count */
    igraph_long_t i;
    ...
    *time += 1;
    ...
    nc = igraph_vector_long_size(incedges);
    for (i = 0; i < nc; ++i) {
        igraph_long_t edge = (igraph_long_t) VECTOR(*incedges)[i];
        igraph_long_t v = IGRAPH_TO(graph, edge) == u ? IGRAPH_FROM(graph, edge) : IGRAPH_TO(graph, edge);
...
igraph_long_t igraph_bridges(const igraph_t *graph, igraph_vector_t *bridges) {
    ...
    igraph_vector_long_t incoming_edge;
    igraph_long_t n;
    igraph_long_t i;
    igraph_long_t time;
    ...
    IGRAPH_CHECK(igraph_vector_long_init(&disc, n));
    IGRAPH_FINALLY(igraph_vector_long_destroy, &disc);

    IGRAPH_CHECK(igraph_vector_long_init(&low, n));
    IGRAPH_FINALLY(igraph_vector_long_destroy, &low);

    IGRAPH_CHECK(igraph_vector_long_init(&incoming_edge, n));
    IGRAPH_FINALLY(igraph_vector_long_destroy, &incoming_edge);
    for (i = 0; i < n; ++i) {
    ...
    igraph_vector_long_destroy(&incoming_edge);
    igraph_vector_long_destroy(&low);
    igraph_vector_long_destroy(&disc);
...
igraph_long_t igraph_subcomponent(const igraph_t *graph, igraph_vector_t *res, igraph_real_t vertex,
    ...
    already_added[(igraph_long_t)vertex] = 1;
    ...
    while (!igraph_dqueue_empty(&q)) {
        ...
        IGRAPH_CHECK(igraph_neighbors(graph, &tmp, (igraph_long_t) actnode,
                                      mode));
