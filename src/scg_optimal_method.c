										...
										const UINT matrix, const REAL *p)
{
	...
	qsort(vs, n, sizeof(INDVAL), compare_ind_val);
	...
	for(i=0; i<n; i++)
		F[0][i] = real_sym_mat_get(Cv,0,i);
		
	for(i=1; i<nt; i++)
		for(j=i+1; j<n; j++){
			F[i][j] = F[i-1][i-1] + real_sym_mat_get(Cv,i,j);
			Q[i][j] = 2;
		
			for(q=i-1; q<=j-1; q++){
				temp = F[i-1][q] + real_sym_mat_get(Cv,q+1,j);
				if(temp<F[i][j]){
					F[i][j] = temp;
					Q[i][j] = q+2;
				}
