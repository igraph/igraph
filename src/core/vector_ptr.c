 ...
 * as for \ref igraph_vector_t. The same \ref VECTOR macro used for ordinary
 * vectors can also be used for pointer vectors as well, but note that a typeless
 * generic pointer will be provided by this macro and you will most likely need
 * to cast it to a specific pointer before starting to work with it.</para>
 *
 * <para>Pointers placed in a pointer vector are \em not owned by the pointer
 * vector itself, and the pointer vector is \em not responsible for freeing them
 * when they are not needed any more. This rule applies even for igraph functions
 * that take a pointer vector as an output argument. For instance, \ref igraph_decompose()
 * uses an \type igraph_vector_ptr_t to return pointers to newly created graph
 * objects during the decomposition, but the ownership of these pointers is
 * passed on to the caller and it is the responsibility of the caller to
 * \em destroy the graphs when they are not needed any more, call \c igraph_free() on
 * the pointers themselves, and then to \em destroy the entire pointer vector
 * with \ref igraph_vector_ptr_destroy().</para>
 *
 ...
 *
 * </para><para>
 * The destructor for pointer vectors. Note that it destroys the pointer vector
 * only, but does \em not destroy any of the contained items. See
 * \ref igraph_vector_ptr_free_items(), \ref igraph_vector_ptr_destroy_items(),
 * \ref igraph_vector_ptr_destroy_and_free_items() and \ref igraph_vector_ptr_destroy_all()
 * if you also want to destroy the contained items and/or free the memory
 ...
 * \brief Frees all the elements of a pointer vector - deprecated alias.
 *
 * \param v Pointer to the pointer vector whose elements will be freed.
 *
 * \deprecated-by igraph_vector_ptr_free_items 0.10.0
 */

void igraph_vector_ptr_free_all(igraph_vector_ptr_t* v) {
    igraph_vector_ptr_free_items(v);
}

/**
 * \ingroup vectorptr
 * \function igraph_vector_ptr_free_items
 * \brief Frees all the elements of a pointer vector.
 *
 * \param v Pointer to the pointer vector whose elements will be freed.
 *
 * Time complexity: operating system dependent, the \quote time
 * \endquote required to deallocate O(n) pointers. n is the number of elements
 * in the pointer vector.
 */

void igraph_vector_ptr_free_items(igraph_vector_ptr_t* v) {
    igraph_vector_ptr_destroy_and_free_items(v, /* func = */ 0);
}

/**
 * \ingroup vectorptr
 * \function igraph_vector_ptr_destroy_items
 * \brief Calls a destructor function on all elements of a pointer vector
 *
 * \param v Pointer to the pointer vector whose elements will be destroyed.
 * \param func The destructor function to call on each element.
 *
 * Time complexity: operating system dependent, the \quote time
 * \endquote required to call the destructor function on O(n) pointers.
 * n is the number of elements in the pointer vector.
 */
void igraph_vector_ptr_destroy_items(igraph_vector_ptr_t* v, igraph_finally_func_t* func) {
    void **ptr;
    IGRAPH_ASSERT(v != 0);
...
/**
 * \ingroup vectorptr
 * \function igraph_vector_ptr_destroy_and_free_items
 * \brief Calls a destructor function on all elements of a pointer vector and frees them
 *
 * \param v Pointer to the pointer vector whose elements will be destroyed and freed.
 * \param func The destructor function to call on each element.
 *
 * Time complexity: operating system dependent, the \quote time
 * \endquote required to call the destructor function on O(n) pointers and then
 * to deallocate the memory associated to these pointers. n is the number of
 * elements in the pointer vector.
 */
void igraph_vector_ptr_destroy_and_free_items(igraph_vector_ptr_t* v, igraph_finally_func_t* func) {
    void **ptr;
    IGRAPH_ASSERT(v != 0);
    IGRAPH_ASSERT(v->stor_begin != 0);
 ...
 *
 * This function is equivalent to \ref igraph_vector_ptr_destroy_and_free_items()
 * followed by \ref igraph_vector_ptr_destroy().
 *
 * \param v Pointer to the pointer vector to destroy.
 * \param func The destructor function to call on each element. May be \c NULL
 *        if there is no need to call a destructor function.
 *
 * Time complexity: operating system dependent, the \quote time
 * \endquote required to call the destructor function on O(n) pointers,
 * plus the \quote time \endquote required to deallocate O(n) pointers, each
 * pointing to a memory area of arbitrary size, plus the \quote time \endquote
...
void igraph_vector_ptr_destroy_all(igraph_vector_ptr_t* v, igraph_finally_func_t* func) {
    igraph_vector_ptr_destroy_and_free_items(v, func ? func : v->item_destructor);
    igraph_vector_ptr_destroy(v);
}

void igraph_i_vector_ptr_destroy_with_item_destructor(igraph_vector_ptr_t *v) {
    igraph_vector_ptr_destroy_all(v, v->item_destructor);
}
...
void igraph_vector_ptr_clear(igraph_vector_ptr_t* v) {
    IGRAPH_ASSERT(v != NULL);
    IGRAPH_ASSERT(v->stor_begin != NULL);
    igraph_vector_ptr_destroy_items(v, v->item_destructor);
    v->end = v->stor_begin;
}
 ...
 * igraph_vector_ptr_destroy_all() or \ref igraph_vector_ptr_free_items()
 * is called.
 *
