...
const char* igraph_strerror(const igraph_long_t igraph_errno) {
    if (igraph_errno < 0 ||
        ((igraph_long_t)igraph_errno) >= sizeof(igraph_i_error_strings) / sizeof(char *)) {
...
void IGRAPH_FINALLY_REAL(void (*func)(void*), void* ptr) {
    igraph_long_t no = igraph_i_finally_stack[0].all;
...
void IGRAPH_FINALLY_CLEAN(igraph_long_t minus) {
...
igraph_long_t IGRAPH_FINALLY_STACK_SIZE(void) {
...
void igraph_fatal(const char *reason, const char *file, igraph_long_t line) {
...
void igraph_fatalf(const char *reason, const char *file, igraph_long_t line, ...) {
    va_list ap;
    va_start(ap, line);
