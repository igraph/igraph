...
igraph_integer_t igraph_2dgrid_which(igraph_2dgrid_t *grid, igraph_real_t xc, igraph_real_t yc,
    ...
    if (xc <= grid->minx) {
        ...
        *x = (igraph_integer_t) floor((xc - (grid->minx)) / (grid->deltax));
    }
    ...
    if (yc <= grid->miny) {
        ...
        *y = (igraph_integer_t) floor((yc - (grid->miny)) / (grid->deltay));
    }

    return 0;
}

igraph_integer_t igraph_2dgrid_init(igraph_2dgrid_t *grid, igraph_matrix_t *coords,
    ...
    grid->stepsx = (igraph_integer_t) ceil((maxx - minx) / deltax);
    grid->stepsy = (igraph_integer_t) ceil((maxy - miny) / deltay);
...
void igraph_2dgrid_add(igraph_2dgrid_t *grid, igraph_integer_t elem,
    ...
    first = (igraph_integer_t) MATRIX(grid->startidx, x, y);
...
void igraph_2dgrid_move(igraph_2dgrid_t *grid, igraph_integer_t elem,
    ...
    if (oldx != newx || oldy != newy) {
        ...
        /* add to this cell */
        first = (igraph_integer_t) MATRIX(grid->startidx, newx, newy);
        VECTOR(grid->prev)[elem] = 0;
        VECTOR(grid->next)[elem] = first;
        if (first != 0) {
            VECTOR(grid->prev)[first - 1] = elem + 1;
...
igraph_integer_t igraph_i_2dgrid_addvertices(igraph_2dgrid_t *grid, igraph_vector_t *eids,
    ...
    r = r * r;
    act = (igraph_integer_t) MATRIX(grid->startidx, x, y);
    while (act != 0) {
        if (igraph_2dgrid_dist2(grid, vid, act - 1) < r) {
            IGRAPH_CHECK(igraph_vector_push_back(eids, act - 1));
        }
        act = (igraph_integer_t) v[act - 1];
    }
    return 0;
}

igraph_integer_t igraph_2dgrid_neighbors(igraph_2dgrid_t *grid, igraph_vector_t *eids,
                            igraph_integer_t vid, igraph_real_t r) {
    igraph_real_t xc = MATRIX(*grid->coords, (igraph_integer_t)vid, 0);
    igraph_real_t yc = MATRIX(*grid->coords, (igraph_integer_t)vid, 1);
    igraph_integer_t x, y;
...
void igraph_2dgrid_reset(igraph_2dgrid_t *grid, igraph_2dgrid_iterator_t *it) {
    ...
    it->x = 0; it->y = 0; it->vid = (igraph_integer_t) MATRIX(grid->startidx, 0, 0);
    while ( it->vid == 0 && (it->x < grid->stepsx - 1 || it->y < grid->stepsy - 1)) {
        it->x += 1;
        if (it->x == grid->stepsx) {
            it->x = 0; it->y += 1;
        }
        it->vid = (igraph_integer_t) MATRIX(grid->startidx, it->x, it->y);
    }
}
...
igraph_integer_t igraph_2dgrid_next(igraph_2dgrid_t *grid,
    ...
    it->nei = (igraph_integer_t) VECTOR(grid->next) [ ret - 1 ];
    while (it->ncells > 0 && it->nei == 0 ) {
        it->ncells -= 1;
        it->nei = (igraph_integer_t) MATRIX(grid->startidx, it->nx[it->ncells], it->ny[it->ncells]);
    }

    /* Next vertex */
    it->vid = (igraph_integer_t) VECTOR(grid->next)[ it->vid - 1 ];
