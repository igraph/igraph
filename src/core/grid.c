...
igraph_error_t igraph_2dgrid_which(igraph_2dgrid_t *grid, igraph_real_t xc, igraph_real_t yc,
    ...
    if (xc <= grid->minx) {
        ...
        *x = (igraph_long_t) floor((xc - (grid->minx)) / (grid->deltax));
    }
    ...
    if (yc <= grid->miny) {
        ...
        *y = (igraph_long_t) floor((yc - (grid->miny)) / (grid->deltay));
    }
...
igraph_error_t igraph_2dgrid_init(igraph_2dgrid_t *grid, igraph_matrix_t *coords,
    ...
    grid->stepsx = (igraph_long_t) ceil((maxx - minx) / deltax);
    grid->stepsy = (igraph_long_t) ceil((maxy - miny) / deltay);
...
void igraph_2dgrid_add(igraph_2dgrid_t *grid, igraph_long_t elem,
    ...
    first = (igraph_long_t) MATRIX(grid->startidx, x, y);
...
void igraph_2dgrid_move(igraph_2dgrid_t *grid, igraph_long_t elem,
                        igraph_real_t xc, igraph_real_t yc) {
    igraph_long_t oldx, oldy;
    igraph_long_t newx, newy;
    ...
    if (oldx != newx || oldy != newy) {
        ...
        if (VECTOR(grid->prev)[elem] != 0) {
            VECTOR(grid->next) [ (igraph_long_t) VECTOR(grid->prev)[elem] - 1 ] =
                VECTOR(grid->next)[elem];
        } else {
            MATRIX(grid->startidx, oldx, oldy) = VECTOR(grid->next)[elem];
        }
        if (VECTOR(grid->next)[elem] != 0) {
            VECTOR(grid->prev)[ (igraph_long_t) VECTOR(grid->next)[elem] - 1 ] =
                VECTOR(grid->prev)[elem];
        }

        /* add to this cell */
        first = (igraph_long_t) MATRIX(grid->startidx, newx, newy);
        VECTOR(grid->prev)[elem] = 0;
        VECTOR(grid->next)[elem] = first;
        if (first != 0) {
            VECTOR(grid->prev)[first - 1] = elem + 1;
...
void igraph_2dgrid_reset(igraph_2dgrid_t *grid, igraph_2dgrid_iterator_t *it) {
    ...
    it->x = 0; it->y = 0; it->vid = (igraph_long_t) MATRIX(grid->startidx, 0, 0);
...
igraph_error_t igraph_2dgrid_next(igraph_2dgrid_t *grid,
    ...
    it->nei = (igraph_long_t) VECTOR(grid->next) [ ret - 1 ];
    while (it->ncells > 0 && it->nei == 0 ) {
        it->ncells -= 1;
        it->nei = (igraph_long_t) MATRIX(grid->startidx, it->nx[it->ncells], it->ny[it->ncells]);
    }

    /* Next vertex */
    it->vid = (igraph_long_t) VECTOR(grid->next)[ it->vid - 1 ];
