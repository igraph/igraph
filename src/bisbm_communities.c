 ...
 *     * do something to deal with degree 0 vertices 
 *        (put them all in community -1 or something? Right now, they get placed
 *        into arbitrary communities.)
 ...
 */
typedef struct {
  /* number of communities of type a, number of communities of type b, number 
  of vertices */
  int a, b, size; 
  ...
  /* matrix in which cell ab is the number of edges between communities a-b */
  igraph_matrix_t *inter_comm_edges;
  /* For each community, the total degree of all vertices in the community. */
 ...
 * 
 * TODO: implement non-degree corrected version (equation 9).
 * TODO: implement change in score instead of total score for a possible
 *       optimization.
 */
static igraph_real_t score_partition(Housekeeping *hk) {
  igraph_real_t score = 0;
  igraph_real_t neginfinity = IGRAPH_NEGINFINITY;
  int c_a;
  int c_b;
  for (c_a = 0; c_a < hk->a; c_a++) {
    for (c_b = hk->a; c_b < hk->a + hk->b; c_b++) {
      ...
      if (!(int) inter_community || !(int) c_a_sum || !(int) c_b_sum) {
        return neginfinity;
      }
 ...
 *                       and community information
 *     int v: The index of the vertex to be swapped for scoring.
 *     int *dest: A pointer to an uninitialized int where the best swap will 
 *                be stored.
 *   Returns: 
 ...
 * Takes a housekeeping struct, a bool array representing whether each vertex 
 * has been used, an array of swaprecords to give the ability to rewind to the
 * best swap, and an int i representing the swap number that we're on. 
 *   Parameters:
 *     Housekeeping *hk: A struct Housekeeping containing all graph 
 *                       and community information
 *     bool *used: An array of length hk->size where used[i] is false if 
 *                 vertex i is in group a, true if group b.
 *     Swaprecord *swaprecord: An array of swaprecords in order, each
 *                             representing one swap.
...
static double make_best_swap(Housekeeping *hk, igraph_bool_t *used, Swaprecord *swaprecord, int i) {
  int max_v = -1;
  double max_score = IGRAPH_NEGINFINITY;
  int max_swap = -1;
  int v;
  for (v = 0; v < hk->size; v++) {
    int dest;
  ...
  if (max_v == -1) {
    /* don't make any swaps -- an empty community is illegal.
     we cheat a little here -- we just record an empty swap on vertex 0. */
    swaprecord[i].v = 0;
 ...
 * swaps vertices back and updates housekeeping to reflect the best swap for 
 * the current iteration.
 */
static void rewind_swaps(Housekeeping *hk, Swaprecord *swaprecord, int best_swap) {
  int i;
  for (i = hk->size - 1; i > best_swap; i--) {
...
static int initialize_partition(Housekeeping *hk) {
  int *partition = igraph_Calloc(hk->size, int);
  int i;
  if (partition==NULL) {
    IGRAPH_ERROR("Unable to allocate space for membership list.", IGRAPH_ENOMEM);
  }
  RNG_BEGIN();
  IGRAPH_FINALLY(igraph_free, partition);
  for (i = 0; i < hk->size; i++) {
    if (!VECTOR(*(hk->types))[i]) {/* type 0 */
      partition[i] = RNG_INTEGER(0, hk->a - 1);
    } else { /* type 1 */
      partition[i] = RNG_INTEGER(hk->a, hk->a + hk->b - 1);
    }
  }
  RNG_END();
...
static int initialize_neighbors(Housekeeping *hk, igraph_t *graph) {
  IGRAPH_CHECK(igraph_adjlist_init(graph, hk->adj_list, IGRAPH_ALL));
  IGRAPH_FINALLY(igraph_adjlist_destroy, hk->adj_list);
  return 0;
}
 ...
 * --------------------------------
 * Initializes an array hk->comm_tot_degree where comm_tot_degree[i] = the 
 * total degree of all vertices in community i.
 */
static int initialize_degree_sums(Housekeeping *hk) {
  ...
  for (v = 0; v < hk->size; v++) {
    comm_degree[hk->partition[v]] += igraph_vector_int_size(igraph_adjlist_get(hk->adj_list, v));
  }
 ...
 * TODO: allow the user to supply an id or something in community a, so they
 * aren't assigned arbitrarily.
 */
...
static void free_housekeeping(Housekeeping *hk) {
  int freed = 0;
  igraph_Free(hk->partition);
  freed++;
  igraph_Free(hk->comm_tot_degree);
  freed++;
  igraph_adjlist_destroy(hk->adj_list);
  freed++;
  igraph_vector_bool_destroy(hk->types);
  freed++;
  igraph_matrix_destroy(hk->inter_comm_edges);
  freed++;
  IGRAPH_FINALLY_CLEAN(freed);
}
 ...
 * \brief Community detection based on a bipartite stochastic block model.
 * 
 ...
 *
 * A free version of the paper is available at http://arxiv.org/abs/1403.2933
 *
 ...
 * \param score A pointer to an igraph_real_t that will store the score attained
 * by running the algorithm. Since this algorithm is stochastic,
...
int igraph_community_bipartite_sbm(igraph_t *graph, igraph_vector_t *membership, 
                                   ...
                                   igraph_integer_t max_iters, igraph_real_t *score) {
  int i;
  Housekeeping hk;
  igraph_vector_bool_t types;
  igraph_matrix_t inter_comm_edges;
  ...
  *score = run_algorithm(&hk, max_iters);
  ...
  for (i = 0; i < hk.size; i++) VECTOR(*membership)[i] = hk.partition[i];
