...
igraph_long_t igraph_transitivity_avglocal_undirected(const igraph_t *graph,
    ...
    for (i = 0; i < no_of_nodes; i++) {
        VECTOR(rank)[ (igraph_long_t) VECTOR(order)[i] ] = no_of_nodes - i - 1;
    }
    ...
    for (nn = no_of_nodes - 1; nn >= 0; nn--) {
        node = (igraph_long_t) VECTOR(order)[nn];
...
igraph_long_t igraph_transitivity_local_undirected2(const igraph_t *graph,
    ...
    igraph_long_t no_of_nodes = igraph_vcount(graph);
    igraph_vit_t vit;
    igraph_long_t nodes_to_calc, affected_nodes;
    igraph_long_t maxdegree = 0;
    igraph_long_t i, j, k, nn;
    ...
    for (i = 0; i < nodes_to_calc; IGRAPH_VIT_NEXT(vit), i++) {
        ...
        neis2 = igraph_lazy_adjlist_get(&adjlist, (igraph_long_t) v);
        neilen = igraph_vector_size(neis2);
        for (j = 0; j < neilen; j++) {
            igraph_long_t nei = (igraph_long_t) VECTOR(*neis2)[j];
    ...
    for (i = 0; i < affected_nodes; i++) {
        igraph_long_t v = (igraph_long_t) VECTOR(avids)[i];
        igraph_vector_t *neis2;
        igraph_long_t deg;
        neis2 = igraph_lazy_adjlist_get(&adjlist, (igraph_long_t) v);
    ...
    for (i = 0; i < affected_nodes; i++) {
        VECTOR(rank)[ (igraph_long_t) VECTOR(order)[i] ] = affected_nodes - i - 1;
    }
    ...
    for (nn = affected_nodes - 1; nn >= 0; nn--) {
        igraph_long_t node = (igraph_long_t) VECTOR(avids) [ (igraph_long_t) VECTOR(order)[nn] ];
        igraph_vector_t *neis1, *neis2;
        igraph_long_t neilen1, neilen2;
        igraph_long_t nodeindex = (igraph_long_t) VECTOR(indexv)[node];
        igraph_long_t noderank = (igraph_long_t) VECTOR(rank) [nodeindex - 1];

        /*     fprintf(stderr, "node %li (indexv %li, rank %li)\n", node, */
        /*      (igraph_long_t)VECTOR(indexv)[node]-1, noderank); */

        IGRAPH_ALLOW_INTERRUPTION();

        neis1 = igraph_lazy_adjlist_get(&adjlist, (igraph_long_t) node);
        ...
        for (i = 0; i < neilen1; i++) {
            igraph_long_t nei = (igraph_long_t) VECTOR(*neis1)[i];
            igraph_long_t neiindex = (igraph_long_t) VECTOR(indexv)[nei];
            igraph_long_t neirank = (igraph_long_t) VECTOR(rank)[neiindex - 1];

            /*       fprintf(stderr, "  nei %li (indexv %li, rank %li)\n", nei, */
            /*        neiindex, neirank); */
            if (neirank > noderank) {
                neis2 = igraph_lazy_adjlist_get(&adjlist, (igraph_long_t) nei);
                neilen2 = igraph_vector_size(neis2);
                for (j = 0; j < neilen2; j++) {
                    igraph_long_t nei2 = (igraph_long_t) VECTOR(*neis2)[j];
                    igraph_long_t nei2index = (igraph_long_t) VECTOR(indexv)[nei2];
                    igraph_long_t nei2rank = (igraph_long_t) VECTOR(rank)[nei2index - 1];
    ...
    for (i = 0; i < nodes_to_calc; i++, IGRAPH_VIT_NEXT(vit)) {
        ...
        igraph_long_t idx = (igraph_long_t) VECTOR(indexv)[node] - 1;
        igraph_vector_t *neis2 = igraph_lazy_adjlist_get(&adjlist,
                                 (igraph_long_t) node);
        igraph_long_t deg = igraph_vector_size(neis2);
...
/* igraph_long_t igraph_transitivity_local_undirected3(const igraph_t *graph, */
/*                    igraph_vector_t *res, */
/*                    const igraph_vs_t vids) { */
...
/*     igraph_long_t n1=igraph_vector_size(neis); */
/*     igraph_real_t triangles=0; */
/*     igraph_real_t triples=(double)n1*(n1-1); */
/*     IGRAPH_ALLOW_INTERRUPTION(); */
/*     for (j=0; j<n1; j++) { */
/*       igraph_long_t node2=VECTOR(*neis)[j]; */
/*       igraph_vector_t *neis2=igraph_lazy_adjlist_get(&adjlist, node2); */
/*       igraph_long_t n2=igraph_vector_size(neis2); */
/*       igraph_long_t l1=0, l2=0; */
/*       while (l1 < n1 && l2 < n2) { */
/*  igraph_long_t nei1=VECTOR(*neis)[l1]; */
/*  igraph_long_t nei2=VECTOR(*neis2)[l2]; */
/*  if (nei1 < nei2) {  */
/*    l1++; */
/*  } else if (nei1 > nei2) { */
/*    l2++; */
 ...
 *        is stored here, in a igraph_long_t list of triples of vertex ids.
...
igraph_long_t igraph_transitivity_undirected(const igraph_t *graph,
    ...
    igraph_adjlist_t allneis;
    igraph_vector_long_t *neis1, *neis2;
    igraph_long_t i, j, neilen1, neilen2;
...
igraph_long_t igraph_transitivity_barrat1(const igraph_t *graph,
    ...
    for (i = 0; !IGRAPH_VIT_END(vit); IGRAPH_VIT_NEXT(vit), i++) {
        igraph_long_t node = IGRAPH_VIT_GET(vit);
        igraph_long_t adjlen1, adjlen2, j, k;
        igraph_real_t triples, triangles;

        IGRAPH_ALLOW_INTERRUPTION();

        adj1 = igraph_lazy_inclist_get(&incident, (igraph_long_t) node);
        adjlen1 = igraph_vector_size(adj1);
        /* Mark the neighbors of the node */
        for (j = 0; j < adjlen1; j++) {
            igraph_long_t edge = (igraph_long_t) VECTOR(*adj1)[j];
        ...
        for (j = 0; j < adjlen1; j++) {
            igraph_long_t edge1 = (igraph_long_t) VECTOR(*adj1)[j];
            igraph_real_t weight1 = VECTOR(*weights)[edge1];
            igraph_long_t v = IGRAPH_OTHER(graph, edge1, node);
            adj2 = igraph_lazy_inclist_get(&incident, (igraph_long_t) v);
            adjlen2 = igraph_vector_size(adj2);
            for (k = 0; k < adjlen2; k++) {
                igraph_long_t edge2 = (igraph_long_t) VECTOR(*adj2)[k];
                igraph_long_t v2 = IGRAPH_OTHER(graph, edge2, v);
...
igraph_long_t igraph_transitivity_barrat4(const igraph_t *graph,
    ...
    igraph_vector_long_t neis;
    igraph_vector_long_t *adj1, *adj2;
    igraph_vector_t actw;
    igraph_long_t i, nn;
    ...
    for (nn = no_of_nodes - 1; nn >= 0; nn--) {
        igraph_long_t adjlen1, adjlen2;
        igraph_real_t triples;
        igraph_long_t node = (igraph_long_t) VECTOR(order)[nn];

        IGRAPH_ALLOW_INTERRUPTION();

        adj1 = igraph_inclist_get(&incident, node);
        adjlen1 = igraph_vector_long_size(adj1);
        triples = VECTOR(degree)[node] * (adjlen1 - 1) / 2.0;
        /* Mark the neighbors of the node */
        for (i = 0; i < adjlen1; i++) {
            igraph_long_t edge = (igraph_long_t) VECTOR(*adj1)[i];
            igraph_long_t nei = IGRAPH_OTHER(graph, edge, node);
        ...
        for (i = 0; i < adjlen1; i++) {
            igraph_long_t edge1 = (igraph_long_t) VECTOR(*adj1)[i];
            igraph_real_t weight1 = VECTOR(*weights)[edge1];
            igraph_long_t nei = IGRAPH_OTHER(graph, edge1, node);
            igraph_long_t j;
            if (VECTOR(rank)[nei] > VECTOR(rank)[node]) {
                adj2 = igraph_inclist_get(&incident, nei);
                adjlen2 = igraph_vector_long_size(adj2);
                for (j = 0; j < adjlen2; j++) {
                    igraph_long_t edge2 = (igraph_long_t) VECTOR(*adj2)[j];
