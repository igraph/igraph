 ...
 */
igraph_long_t igraph_static_fitness_game(igraph_t *graph, igraph_long_t no_of_edges,
                               igraph_vector_t* fitness_out, igraph_vector_t* fitness_in,
                               igraph_bool_t loops, igraph_bool_t multiple) {
    igraph_vector_t edges = IGRAPH_VECTOR_NULL;
    igraph_long_t no_of_nodes;
    igraph_long_t outnodes, innodes, nodes;
    ...
    float num_steps;
    igraph_long_t step_counter = 0;
    igraph_long_t i, from, to, pos;
    ...
    no_of_nodes = (igraph_long_t) igraph_vector_size(fitness_out);
    if (no_of_nodes == 0) {
    ...
    if (multiple) {
        ...
        while (no_of_edges > 0) {
            ...
            if (igraph_vector_long_binsearch(neis, to, &pos)) {
                continue;
            }

            /* Insert the edge */
            IGRAPH_CHECK(igraph_vector_long_insert(neis, pos, to));
 ...
 * Time complexity: O(|V| + |E| log |E|).
 */
igraph_long_t igraph_static_power_law_game(igraph_t *graph,
