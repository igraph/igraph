...
igraph_error_t igraph_correlated_game(const igraph_t *old_graph, igraph_t *new_graph,
    ...
    if (corr == 0) {
        return igraph_erdos_renyi_game_gnp(new_graph, no_of_nodes, p, directed, IGRAPH_NO_LOOPS, IGRAPH_NO_MULTIPLE);
    }
    if (corr == 1) {
...
igraph_error_t igraph_correlated_pair_game(igraph_t *graph1, igraph_t *graph2,
    ...
    IGRAPH_CHECK(igraph_erdos_renyi_game_gnp(graph1, n, p, directed, IGRAPH_NO_LOOPS, IGRAPH_NO_MULTIPLE));
