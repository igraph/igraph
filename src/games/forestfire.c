 ...
 */
igraph_error_t igraph_forest_fire_game(igraph_t *graph, igraph_long_t nodes,
                            igraph_real_t fw_prob, igraph_real_t bw_factor,
                            igraph_long_t pambs, igraph_bool_t directed) {

    igraph_vector_long_t visited;
    igraph_long_t no_of_nodes = nodes, actnode, i;
    igraph_vector_t edges;
    igraph_vector_t *inneis, *outneis;
    igraph_i_forest_fire_data_t data;
    igraph_dqueue_t neiq;
    igraph_long_t ambs = pambs;
...
#define ADD_EDGE_TO(nei) \
    ...
    for (actnode = 1; actnode < no_of_nodes; actnode++) {
        ...
        /* Choose ambassador(s) */
        for (i = 0; i < ambs; i++) {
            igraph_long_t a = RNG_INTEGER(0, actnode - 1);
            ADD_EDGE_TO(a);
        }

        while (!igraph_dqueue_empty(&neiq)) {
            igraph_long_t actamb = (igraph_long_t) igraph_dqueue_pop(&neiq);
            igraph_vector_t *outv = outneis + actamb;
            igraph_vector_t *inv = inneis + actamb;
            igraph_long_t no_in = igraph_vector_size(inv);
            igraph_long_t no_out = igraph_vector_size(outv);
            igraph_long_t neis_out = (igraph_long_t) RNG_GEOM(param_geom_out);
            igraph_long_t neis_in = (igraph_long_t) RNG_GEOM(param_geom_in);
            /* outgoing neighbors */
            if (neis_out >= no_out) {
                for (i = 0; i < no_out; i++) {
                    igraph_long_t nei = (igraph_long_t) VECTOR(*outv)[i];
                    ADD_EDGE_TO(nei);
                }
            } else {
                igraph_long_t oleft = no_out;
                for (i = 0; i < neis_out && oleft > 0; ) {
                    igraph_long_t which = RNG_INTEGER(0, oleft - 1);
                    igraph_long_t nei = (igraph_long_t) VECTOR(*outv)[which];
            ...
            if (neis_in >= no_in) {
                for (i = 0; i < no_in; i++) {
                    igraph_long_t nei = (igraph_long_t) VECTOR(*inv)[i];
                    ADD_EDGE_TO(nei);
                }
            } else {
                igraph_long_t ileft = no_in;
                for (i = 0; i < neis_in && ileft > 0; ) {
                    igraph_long_t which = RNG_INTEGER(0, ileft - 1);
                    igraph_long_t nei = (igraph_long_t) VECTOR(*inv)[which];
