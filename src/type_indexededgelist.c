...
int igraph_add_edges(igraph_t *graph, const igraph_vector_t *edges,
  ...
  ret1=igraph_i_vector_order(&graph->from, &graph->to, &newoi, graph->n);
  ret2=igraph_i_vector_order(&graph->to  , &graph->from, &newii, graph->n);
  if (ret1 != 0 || ret2 != 0) {
...
int igraph_delete_edges(igraph_t *graph, igraph_es_t edges) {
  ...
  IGRAPH_CHECK(igraph_i_vector_order(&newfrom, &newto, &newoi, no_of_nodes));
  IGRAPH_CHECK(igraph_i_vector_order(&newto, &newfrom, &graph->ii, no_of_nodes));
...
int igraph_delete_vertices(igraph_t *graph, const igraph_vs_t vertices) {
  ...
  /* update oi & ii */
  IGRAPH_CHECK(igraph_i_vector_order(&newgraph.from, &newgraph.to, &newgraph.oi, 
				   remaining_vertices));
  IGRAPH_CHECK(igraph_i_vector_order(&newgraph.to, &newgraph.from, &newgraph.ii, 
				   remaining_vertices));  
