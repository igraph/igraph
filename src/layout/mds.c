...
igraph_long_t igraph_i_layout_mds_single(const igraph_t* graph, igraph_matrix_t *res,
                               igraph_matrix_t *dist, igraph_long_t dim) {

    igraph_long_t no_of_nodes = igraph_vcount(graph);
    igraph_long_t nev = dim;
...
igraph_long_t igraph_layout_mds(const igraph_t* graph, igraph_matrix_t *res,
    ...
    if (conn) {
        ...
        igraph_long_t j, n, processed_vertex_count = 0;
        ...
        for (i = 0; i < no_of_nodes; i++) {
            ...
            for (j = 0; j < n; j++) {
                seen_vertices[(igraph_long_t)VECTOR(comp)[j]] = 1;
                VECTOR(vertex_order)[(igraph_long_t)VECTOR(comp)[j]] = processed_vertex_count++;
            }
        }
