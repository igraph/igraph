...
/*
//################################################################################
//Here we sucessively add nodes of positive affinity to the group
//################################################################################
double PottsModel::FindCommunityFromStart(double gamma, double prob, char *nodename)
{
  ...
  NNode *start_node, *n_cur, *neighbor, *max_aff_node, *min_aff_node, *node;
  NLink *l_cur;
  bool found=false, add=false, remove=false;
  double degree, affinity, max_affinity, min_affinity, Ks=0.0, w;
...
//      fprintf(file,"%s not found found. Aborting.\n",nodename);
  ...
  //#############################
  // initialize community with the neighbors of the the start node and 
  // the to_do list with the second shell of start node
  //#############################
...
// 	printf("Adding node %s to to_do list.\n",neighbor->Get_Name());
  ...
  while (add || remove) {
      //#############################
      //calculate the affinities off all nodes in the surroundings of the community
      //##############################
      max_affinity=-1e12;
      max_aff_node=NULL;
      add=false;
	...
	affinity=affinity-gamma*degree/total_degree_sum*Ks;
	node->Set_Affinity(affinity);
	if (node->Get_Affinity()>max_affinity && node->Get_Affinity()>=0.0) {
	    max_affinity=node->Get_Affinity();
	    max_aff_node=node;
	    add=true;
	}
	...
	node=iter.Next();
      ...
      //################
      //calculate the affinities of all the nodes in the community
      //################
      min_affinity=0;
      min_aff_node=NULL;
      inner_links=0;
	...
	affinity=affinity-gamma*degree/total_degree_sum*(Ks-degree);
	node->Set_Affinity(affinity);
	// we should not remove the nodes, we have just added
	if (node->Get_Affinity()<min_affinity) {
	    min_affinity=node->Get_Affinity();
	    min_aff_node=node;
	    remove=true;
	}
	//printf("%s in to_do list with affinity %f\n",node->Get_Name(),node->Get_Affinity());
	node=iter.Next();
      ...
      //################
      if (add && max_affinity>=fabs(min_affinity)) {
	  //################
...
// 	    printf("Adding node %s to community with affinity of %f.\n",max_aff_node->Get_Name(), max_aff_node->Get_Affinity());
      ...
      if (remove && fabs(min_affinity)>=max_affinity) {
	  //################
	  //remove those with negative affinities
	  //################
	  if (remove) {
	    community->fDelete(min_aff_node);
	    min_aff_node->Set_Marker(to_do_marker);
	    //update the sum of degrees in the community
	    Ks-=min_aff_node->Get_Weight();
	    //add the node to to_do again
	    to_do->Push(min_aff_node);
// 	    printf("Removing node %s from community with affinity of %f.\n",min_aff_node->Get_Name(), min_aff_node->Get_Affinity());
	  }
      }
  }    
  //###################
  ...
  //###################
  // TODO: return this somehow instead of writing it
