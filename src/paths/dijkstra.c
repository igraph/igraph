 ...
 */
igraph_long_t igraph_shortest_paths_dijkstra(const igraph_t *graph,
                                   igraph_matrix_t *res,
        ...
        while (!igraph_2wheap_empty(&Q)) {
            ...
            if (all_to) {
                ...
                if (VECTOR(indexv)[minnei]) {
                    MATRIX(*res, i, (igraph_long_t)(VECTOR(indexv)[minnei] - 1)) = mindist - 1.0;
                    reached++;
                    if (reached == no_of_to) {
                        igraph_2wheap_clear(&Q);
                        break;
            ...
            neis = igraph_lazy_inclist_get(&inclist, (igraph_long_t) minnei);
            nlen = igraph_vector_size(neis);
            for (j = 0; j < nlen; j++) {
                igraph_long_t edge = (igraph_long_t) VECTOR(*neis)[j];
                igraph_long_t tto = IGRAPH_OTHER(graph, edge, minnei);
 ...
 */
igraph_long_t igraph_get_shortest_paths_dijkstra(const igraph_t *graph,
    ...
    */
    ...
    igraph_long_t *parents;
    igraph_bool_t *is_target;
    igraph_long_t i, to_reach;
    ...
    parents = igraph_Calloc(no_of_nodes, igraph_long_t);
    if (parents == 0) {
    ...
    for (IGRAPH_VIT_RESET(vit); !IGRAPH_VIT_END(vit); IGRAPH_VIT_NEXT(vit)) {
        if (!is_target[ (igraph_long_t) IGRAPH_VIT_GET(vit) ]) {
            is_target[ (igraph_long_t) IGRAPH_VIT_GET(vit) ] = 1;
        } else {
            to_reach--;       /* this node was given multiple times */
        }
    }

    VECTOR(dists)[(igraph_long_t)from] = 0.0;  /* zero distance */
    parents[(igraph_long_t)from] = 0;
    igraph_2wheap_push_with_index(&Q, from, 0);

    while (!igraph_2wheap_empty(&Q) && to_reach > 0) {
        igraph_long_t nlen, minnei = igraph_2wheap_max_index(&Q);
        ...
        nlen = igraph_vector_size(neis);
        for (i = 0; i < nlen; i++) {
            igraph_long_t edge = (igraph_long_t) VECTOR(*neis)[i];
            igraph_long_t tto = IGRAPH_OTHER(graph, edge, minnei);
            igraph_real_t altdist = mindist + VECTOR(*weights)[edge];
...
igraph_long_t igraph_get_shortest_path_dijkstra(const igraph_t *graph,
 ...
 */
igraph_long_t igraph_get_all_shortest_paths_dijkstra(const igraph_t *graph,
    ...
    */
    ...
    while (!igraph_2wheap_empty(&Q) && to_reach > 0) {
        ...
        printf("Reached vertex %ld, is_target[%ld] = %d, %ld to go\n",
            minnei, minnei, (igraph_long_t)is_target[minnei], to_reach - is_target[minnei]);
        */
    ...
    */
    ...
    if (nrgeo) {
         ...
         * build nrgeo here.
         */
        VECTOR(*nrgeo)[(igraph_long_t)from] = 1;
        n = igraph_vector_size(&order);
        for (i = 1; i < n; i++) {
            igraph_long_t node, j, k;
            igraph_vector_t *parent_vec;

            node = (igraph_long_t)VECTOR(order)[i];
            /* now, take the parent vertices */
            parent_vec = (igraph_vector_t*)VECTOR(parents)[node];
            k = igraph_vector_size(parent_vec);
            for (j = 0; j < k; j++) {
                VECTOR(*nrgeo)[node] += VECTOR(*nrgeo)[(igraph_long_t)VECTOR(*parent_vec)[j]];
            }
        }
    }
    ...
    if (res) {
        ...
        if (igraph_vs_is_all(&to)) {
            ...
            IGRAPH_FINALLY(igraph_vit_destroy, &vit);
            for (IGRAPH_VIT_RESET(vit); !IGRAPH_VIT_END(vit); IGRAPH_VIT_NEXT(vit)) {
                i = (igraph_long_t) IGRAPH_VIT_GET(vit);
                if (!is_target[i]) {
                    is_target[i] = 1;
        ...
        VECTOR(*path)[0] = from;
        VECTOR(*paths_index)[(igraph_long_t)from] = 1;

        for (i = 1; i < n; i++) {
            igraph_long_t m, path_count;
            ...
            */
            ...
            for (j = 0; j < m; j++) {
                ...
                igraph_long_t parent_node = (igraph_long_t) VECTOR(*parent_vec)[j];
                igraph_long_t parent_path_idx = (igraph_long_t) VECTOR(*paths_index)[parent_node] - 1;
                /*
        ...
        for (i = 0; i < n; i++) {
            ...
            if (is_target[(igraph_long_t)tmp] == 1) {
