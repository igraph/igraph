...
*/
static igraph_long_t igraph_i_is_eulerian_undirected(const igraph_t *graph, igraph_bool_t *has_path, igraph_bool_t *has_cycle, igraph_long_t *start_of_path) {
    igraph_long_t odd;
    igraph_vector_t degree, csize;
    /* boolean vector to mark singletons: */
    igraph_vector_t nonsingleton;
    igraph_long_t i, n, vsize;
    igraph_long_t cluster_count;
    ...
    ens = 0;
    for (i = 0; i < n; i++) {
        igraph_long_t deg = (igraph_long_t) VECTOR(degree)[i];
    ...
    for (i = 0; i < n; i++) {
        if ((*has_cycle && ((igraph_long_t) VECTOR(degree)[i]) > 0) || (!*has_cycle && ((igraph_long_t) VECTOR(degree)[i]) %2 == 1)) {
            *start_of_path = i;
            break;
        }
    }
...
static igraph_long_t igraph_i_is_eulerian_directed(const igraph_t *graph, igraph_bool_t *has_path, igraph_bool_t *has_cycle, igraph_long_t *start_of_path) {
    igraph_bool_t res_weak;
    igraph_long_t incoming_excess, outgoing_excess, n;
    ...
    *start_of_path = -1;
    for (i = 0; i < n; i++) {
        igraph_long_t degin = VECTOR(in_degree)[i];
        igraph_long_t degout = VECTOR(out_degree)[i];
        ...
        /* as igraph_long_t as we have perfect balance, you can start
...
static igraph_long_t igraph_i_eulerian_path_undirected(const igraph_t *graph, igraph_vector_t *edge_res, igraph_vector_t *vertex_res, igraph_long_t start_of_path) {
    igraph_long_t curr;
    igraph_long_t n, m;
    igraph_inclist_t il;
    ...
    while (!igraph_stack_empty(&tracker)) {
        ...
        if (VECTOR(degree)[curr] != 0) {
            ...
            igraph_long_t nc, edge;
            igraph_long_t j, next;
            ...
            for (j = 0; j < nc; j++) {
                edge = (igraph_long_t) VECTOR(*incedges)[j];
...
static igraph_long_t igraph_i_eulerian_path_directed(const igraph_t *graph, igraph_vector_t *edge_res, igraph_vector_t *vertex_res, igraph_long_t start_of_path) {
    igraph_long_t curr;
    igraph_long_t n, m;
    igraph_inclist_t il;
    igraph_stack_t path, tracker, edge_tracker, edge_path;
