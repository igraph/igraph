...
igraph_long_t igraph_get_all_shortest_paths(const igraph_t *graph,
    ...
    igraph_long_t n, j, i;
    igraph_long_t to_reach, reached = 0, maxdist = 0;
     ...
     * source */
    geodist = igraph_Calloc(no_of_nodes, igraph_long_t);
    if (geodist == 0) {
    ...
    for (IGRAPH_VIT_RESET(vit); !IGRAPH_VIT_END(vit); IGRAPH_VIT_NEXT(vit)) {
        if (geodist[ (igraph_long_t) IGRAPH_VIT_GET(vit) ] == 0) {
            geodist[ (igraph_long_t) IGRAPH_VIT_GET(vit) ] = -1;
        } else {
            to_reach--;       /* this node was given multiple times */
        }
    }

    if (geodist[ (igraph_long_t) from ] < 0) {
        reached++;
    }
    ...
    VECTOR(*vptr)[0] = from;
    geodist[(igraph_long_t)from] = 1;
    VECTOR(ptrhead)[(igraph_long_t)from] = 1;
    ...
    while (!igraph_dqueue_empty(&q)) {
        ...
        for (j = 0; j < n; j++) {
            ...
            fatherptr = (igraph_long_t) VECTOR(ptrhead)[actnode];
            while (fatherptr != 0) {
                ...
                fatherptr = (igraph_long_t) VECTOR(ptrlist)[fatherptr - 1];
            }
        }
    }
    ...
    memset(geodist, 0, sizeof(igraph_long_t) * (size_t) no_of_nodes);
    for (IGRAPH_VIT_RESET(vit); !IGRAPH_VIT_END(vit); IGRAPH_VIT_NEXT(vit)) {
        geodist[ (igraph_long_t) IGRAPH_VIT_GET(vit) ] = 1;
    }
    ...
    for (i = 0; i < no_of_nodes; i++) {
        igraph_long_t fatherptr = (igraph_long_t) VECTOR(ptrhead)[i];
        if (geodist[i] > 0) {
            while (fatherptr != 0) {
                n++;
                fatherptr = (igraph_long_t) VECTOR(ptrlist)[fatherptr - 1];
