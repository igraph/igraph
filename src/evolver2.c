...
int igraph_revolver_mes_d_d(const igraph_t *graph, 
  ...
  for (timestep=0; timestep<no_of_events; timestep++) {
    ...
    while (eptr < no_of_edges && 
      ...
      xidx=VECTOR(degree)[from];
      yidx=VECTOR(degree)[to];
	...
	if (VECTOR(added)[edge]) {
	  ...
	  if (NTKK(xidx, deg)==1) {
	    MATRIX(ch, deg, xidx+1) = eptr_new;
	    MATRIX(ch, xidx+1, deg) = MATRIX(ch, deg, xidx+1);
	  }
	  MATRIX(ntkk, xidx+1, deg) += 1;
	  MATRIX(ntkk, deg, xidx+1) = MATRIX(ntkk, xidx+1, deg);
	  if (NTKK(xidx+1, deg)==0) {
	    MATRIX(*normfact, xidx, deg) += eptr_new-MATRIX(ch, xidx, deg);
	    MATRIX(*normfact, deg, xidx) = MATRIX(*normfact, xidx, deg);
	  }
	}
      }
	...
	if (VECTOR(added)[edge]) {
	  ...
	  if (NTKK(yidx, deg)==1) {
	    MATRIX(ch, deg, yidx+1) = eptr_new;
	    MATRIX(ch, yidx+1, deg) = MATRIX(ch, deg, yidx+1);
	  }
	  MATRIX(ntkk, yidx+1, deg) += 1;
	  MATRIX(ntkk, deg, yidx+1) = MATRIX(ntkk, yidx+1, deg);
	  if (NTKK(yidx+1, deg)==0) {
	    MATRIX(*normfact, yidx, deg) += eptr_new-MATRIX(ch, yidx, deg);
	    MATRIX(*normfact, deg, yidx) = MATRIX(*normfact, yidx, deg);
	  }
	}
      }
      ...
      if (NTKK(xidx+1, xidx+1)==1) {
	MATRIX(ch, xidx+1, yidx+1) = eptr_new;
	MATRIX(ch, yidx+1, xidx+1) = MATRIX(ch, xidx+1, yidx+1);
      }
