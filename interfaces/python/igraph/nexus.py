...
class NexusConnection(object):
	...
	def list(self, tags=None, operator="or", order="date"):
		"""Retrieves a list of matching datasets from Nexus.
		...
		return NexusDatasetInfoList(self, params)
...
class NexusDatasetInfoList(object):
	...
	def _fetch_results(self, index):
		...
		response = self._conn._get_response("/api/dataset_info", self._params)
