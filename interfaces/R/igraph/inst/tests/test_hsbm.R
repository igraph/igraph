...
test_that("HSBM with 1 cluster per block works", {
  ...
  g <- hsbm.game(10, 5, rho=1, C=matrix(0), p=1)
...
test_that("HSBM with list arguments works", {
  ...
  set.seed(42)
  g <- hsbm.game(b*m, m, rho=rho, C=C, p=0)

  set.seed(42)
  g2 <- hsbm.game(b*m, rep(m, b), rho=rho, C=C, p=0)
  expect_that(g[], equals(g2[]))

  set.seed(42)
  g3 <- hsbm.game(b*m, m, rho=replicate(b, rho, simplify=FALSE), C=C, p=0)
  expect_that(g[], equals(g3[]))

  set.seed(42)
  g4 <- hsbm.game(b*m, m, rho=rho, C=replicate(b, C, simplify=FALSE), p=0)
  expect_that(g[], equals(g4[]))

  expect_that(hsbm.game(b*m, rep(m, b), rho=list(rho, rho), C=C, p=0),
  ...
  gg1 <- hsbm.game(21, m=c(3, 10, 5, 3), rho=list(rho1, rho2, rho3, rho4),
                   C=list(C1, C2, C3, C4), p=1)
  expect_that(is.simple(gg1), is_true())

  set.seed(42)
  gg11 <- hsbm.game(21, m=c(3, 10, 5, 3), rho=list(rho1, rho2, rho3, rho4),
  ...
  gg2 <- hsbm.game(21, m=c(3, 10, 5, 3), rho=list(rho1, rho2, rho3, rho4),
                   C=list(C1, C2, C3, C4), p=0)
  expect_that(is.simple(gg2), is_true())

  gg22 <- hsbm.game(21, m=c(3, 10, 5, 3), rho=list(rho1, rho2, rho3, rho4),
                   C=list(C1, C2, C3, C4), p=1)
