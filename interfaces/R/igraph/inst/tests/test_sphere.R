...
test_that("Sampling sphere surface works", {
  ...
  set.seed(42)
  s1 <- sample.sphere.surface(4, 100, positive=FALSE)
  expect_that(colSums(s1^2), equals(rep(1, 100)))

  s2 <- sample.sphere.surface(3, 100, radius=2, positive=FALSE)
  expect_that(sqrt(colSums(s2^2)), equals(rep(2, 100)))

  s3 <- sample.sphere.surface(2, 100, radius=1/2, positive=TRUE)
...
test_that("Sampling sphere volume works", {
  ...
  set.seed(42)
  s1 <- sample.sphere.volume(4, 10000, positive=FALSE)
  expect_that(all(colSums(s1^2) < 1), is_true())

  s2 <- sample.sphere.volume(3, 100, radius=2, positive=FALSE)
  expect_that(all(sqrt(colSums(s2^2)) < 2), is_true())

  s3 <- sample.sphere.volume(2, 100, radius=1/2, positive=TRUE)
