...
test_that("sample_pa can start from a graph", {
  ...
  g4 <- sample_pa(10, m=1, algorithm="bag", start.graph=empty_graph(5))
  expect_that(ecount(g4), equals(5))
  expect_that(vcount(g4), equals(10))
  expect_that(degree(g4), equals(c(2,0,0,0,1,2,1,1,2,1)))

  g6 <- sample_pa(10, m=1, algorithm="bag", start.graph=star(10))
  expect_that(graph.isomorphic(g6, star(10)), is_true())
  
  g7 <- sample_pa(10, m=3, algorithm="psumtree-multiple",
                start.graph=empty_graph(5))
  ...
  expect_that(graph.isomorphic(induced_subgraph(g8, 1:5), star(5)),
              is_true())

  g9 <- sample_pa(10, m=3, algorithm="psumtree-multiple",
                start.graph=star(10))
  expect_that(graph.isomorphic(g9, star(10)), is_true())

  g10 <- sample_pa(10, m=3, start.graph=empty_graph(5))
  expect_that(degree(g10, mode="out"), equals(c(0,0,0,0,0, 3,3,3,3,3)))

  g11 <- sample_pa(10, m=3, start.graph=star(5))
  expect_that(degree(g11, mode="out"), equals(c(0,1,1,1,1, 3,3,3,3,3)))
  expect_that(graph.isomorphic(induced_subgraph(g11, 1:5), star(5)),
              is_true())
  
  g12 <- sample_pa(10, m=3, start.graph=star(10))
  expect_that(graph.isomorphic(g12, star(10)), is_true())
})
